...
(function (window) {
  ...
  var prefix = 'p__hack_', spfix = 'm-_-c',
    ...
    encode = function (key) {
      key && (key = typeof key === 'string' ? key : JSON.stringify(key))
      return key ? Base64 ? Base64.encode(key) : encodeURIComponent(prefix + key).replace(/%/g, spfix) : ''
    },
    decode = function (key) {
      key && (key = typeof key === 'string' ? key : JSON.stringify(key))
      return key ? Base64 ? Base64.decode(key) : decodeURIComponent(key.replace(reg2, '%')).replace(reg1, '') : ''
    };
  ...
  (function () {
    ...
    //继续探测userData
    var o = document.getElementsByTagName('head')[0], hostKey = window.location.hostname || 'localStorage',
    ...
    LS = {
      ...
      getItem: function (key) {
        ...
        var value = (attrs.getNamedItem(encode(key)) || {nodeValue: null}).nodeValue || root.getAttribute(encode(key))
  ...
  window.LS = !LS ? notSupport : {
    ...
    each: function (callback) {
      var list = this.obj(), fn = callback || function () {
      }, key
      for (key in list) {
