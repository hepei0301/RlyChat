(function (root, factory) {
  ...
  var defaultSettings = {
    ...
    watermark_parent_node: document.body, //水印插件挂载的父元素element,不输入则默认挂在body上
  ...
  var settingsToDefaultSetting = function (settings) {
    ...
    defaultSettings.watermark_pic = settings.watermark_pic || defaultSettings.watermark_pic;
  ...
  loadMark = function (settings) {
    ...
    /*如果元素存在则移除*/
    var watermark_element = defaultSettings.watermark_parent_node.querySelector('#wm_div_id');
    ...
    var watermark_parent_element = defaultSettings.watermark_parent_node;
    if (!watermark_parent_element) return; // 没有水印插件挂载的父元素，不渲染水印内容
    ...
    // console.log(watermark_hook_element, '0-0-0-0-0-')
    ...
    /*创建水印外壳div*/
    var otdiv = defaultSettings.watermark_parent_node.querySelector('#wm_div_id');
    var shadowRoot = null;
    ...
    if (!otdiv) {
      ...
      /*创建shadow dom*/
      otdiv.id = 'wm_div_id';
      otdiv.setAttribute('style', 'pointer-events: none !important; display: block !important; position:absolute !important;');
    ...
    defaultSettings.watermark_cols = parseInt(
      (page_width - defaultSettings.watermark_x) / (defaultSettings.watermark_width + defaultSettings.watermark_x_space) || 0
    );
    var temp_watermark_x_space = parseInt(
      (page_width - defaultSettings.watermark_x - defaultSettings.watermark_width * defaultSettings.watermark_cols) / defaultSettings.watermark_cols
    );
    ...
    defaultSettings.watermark_rows = parseInt(
      (page_height - defaultSettings.watermark_y) / (defaultSettings.watermark_height + defaultSettings.watermark_y_space) || 0
    );
    var temp_watermark_y_space = parseInt(
      (page_height - defaultSettings.watermark_y - defaultSettings.watermark_height * defaultSettings.watermark_rows) / defaultSettings.watermark_rows
    );
    ...
    for (var i = 0; i < defaultSettings.watermark_rows; i++) {
      ...
      for (var j = 0; j < defaultSettings.watermark_cols; j++) {
        ...
        var elementNode = document.createTextNode(defaultSettings.watermark_txt);
        pNode.appendChild(elementNode);
        pNode.style.opacity = defaultSettings.watermark_alpha;
        pNode.style.fontSize = defaultSettings.watermark_fontsize;
        pNode.style.fontFamily = defaultSettings.watermark_font;
        pNode.style.color = defaultSettings.watermark_color;
        pNode.style.textAlign = 'center';
        mask_div.appendChild(pNode);
        if (!!defaultSettings.watermark_pic) {
          var imgNode = document.createElement('img');
          imgNode.src = defaultSettings.watermark_pic;
          imgNode.style.width = defaultSettings.watermark_width - 10 + 'px';
          mask_div.appendChild(imgNode);
        }
        /*设置水印相关属性start*/
    ...
    if (minotor) {
      ...
      if (document.getElementById('wm_div_id')) watermarkDom.observe(document.getElementById('wm_div_id').shadowRoot, option);
    }
  };
