  ...
  var YTX_CONFIG = {
    ...
    _noticeApp: function (obj) {
      ...
      } else if (obj.msgType == YTX_CONFIG._prototype._msg_DEL) { //阅后即焚删除消息
      ...
      } else if (obj.msgType == YTX_CONFIG._prototype._chatRoom.ChatRoom_MESSAGE) { //聊天室推送消息
    ...
    _onResponse: function (obj) {
      ...
      if (type == YTX_CONFIG._prototype._callRoute) {
        ...
        return;
      } else if (type == YTX_CONFIG._prototype._pushMsg) { //普通文本消息和附件走这个分支
        ...
        YTX_CONFIG._processSyncMsg(1);
        return;
      } else if (type == YTX_CONFIG._prototype._syncMsg) { //登陆的时候发现最大版本号和已读版本号不一致会发送一个拉取消息，该分支是响应。
    ...
    _getServerIp: function (type, callback, onError, sig, timestamp, reset) {
      ...
      YTX_CONFIG.jsonp(atob(YTX_CONFIG._app_server) + w + accunt + "/GetServerBalance", datas, function (e) {
    ...
    _initScoket: function (type, callback, onError, sig, timestamp, reset) {
      ...
      window.onbeforeunload = function (event) {
        ...
        YTX_CONFIG._loginStatus = 3;
        if (YTX_CONFIG._socket && YTX_CONFIG._socket.close)
          YTX_CONFIG._socket.close();
    ...
    _protobuf: {
      ...
      _buildSendFileMsgEnd: function (callback, onError, msgId, orignMsgId, isAvatar) {
        if (!isAvatar) {
          ...
          var jsonStr = '{\"1\":' + YTX_CONFIG._generateClientNo(callback, onError, msgId, '', '', '', orignMsgId, true) + '}';
          return '{\"end\":' + jsonStr + ',\"1\":\"' + YTX_CONFIG._reqAuth + '\",\"2\":\"' + YTX_CONFIG._reqSig + '\",\"3\":\"' + YTX_CONFIG._userAcc + '\"}'
        }
      },
      ...
      _CallEventData: function (callEvent, callId, isVoipCall, called, caller, userData, strSDP, reason, mediaInfo, deviceId, clientNo) {
      ...
      _buildCallEvent: function (CallEventBuilder, callback, onError) {
        ...
        if (callstr) {
          ...
          return '{\"MsgLite\":{\"1\":' + YTX_CONFIG._prototype._callRoute + ',\"2\":' + sendProto + ',\"3\":' + (clientNo) + ',\"9":\"' + router + '\"}}'
      ...
      _buildGetFriend: function (getFriendBuilder, callback, onError) {
        ...
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"getFriends\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildAddFriend: function (addFriendsBuilder, callback, onError) {
        ...
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"addFriend\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildetSetFriendRemark: function (setFriendRemark, callback, onError) {
        ...
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"setFriendRemark\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildDelFriend: function (delFriendBuilder, callback, onError) {
        ...
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"delFriend\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildGetRequestHistoryListReq: function (getRequestHistoryListBuilder, callback, onError) {
        ...
        var sendJsonStr = new Object();
        sendJsonStr["useracc"] = YTX_CONFIG._userAcc;
        getRequestHistoryListBuilder.getTimestamp() ? sendJsonStr["timestamp"] = getRequestHistoryListBuilder.getTimestamp() : "";
        getRequestHistoryListBuilder.getSize() ? sendJsonStr["size"] = getRequestHistoryListBuilder.getSize() : "";
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"friendMessage\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildGetOtherInfoReq: function (getOtherInfoBuilder, callback, onError) {
        ...
        var sendJsonStr = new Object();
        sendJsonStr["useracc"] = YTX_CONFIG._userAcc;
        sendJsonStr["searchContent"] = YTX_CONFIG._appid + "#" + getOtherInfoBuilder.getSearchContent();
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"getPersonInfo\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildSetUserVerifyReq: function (setUserVerifyBuilder, callback, onError) {
        ...
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"setUserVerify\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildAgreeFriendRequestReq: function (agreeFriendRequestBuilder, callback, onError) {
        ...
        var sendJsonStr = new Object();
        sendJsonStr["useracc"] = YTX_CONFIG._userAcc;
        sendJsonStr["friendUseracc"] = YTX_CONFIG._appid + "#" + agreeFriendRequestBuilder.getFriendUseracc();
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"friendAgree\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildRefuseFriendRequestReq: function (refuseFriendRequestBuilder, callback, onError) {
        ...
        var sendJsonStr = new Object();
        sendJsonStr["useracc"] = YTX_CONFIG._userAcc;
        sendJsonStr["friendUseracc"] = YTX_CONFIG._appid + "#" + refuseFriendRequestBuilder.getFriendUseracc();
        sendJsonStr["message"] = refuseFriendRequestBuilder.getMessage();
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"friendRefuse\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildGetFriendInfoReq: function (getFriendInfoBuilder, callback, onError) {
        ...
        var sendJsonStr = new Object();
        sendJsonStr["useracc"] = YTX_CONFIG._userAcc;
        sendJsonStr["friendUseracc"] = YTX_CONFIG._appid + "#" + getFriendInfoBuilder.getFriendUseracc();
        sendJsonStr = JSON.stringify(sendJsonStr);
        var clientNo = YTX_CONFIG._generateClientNo(callback, onError);
        return '{\"request\":{\"1\":\"getFriendInfo\",\"2\":\"' + YTX_CONFIG._userAcc + '\",\"3\":' + sendJsonStr + ', \"4\":\"' + YTX_CONFIG._reqSig + '\",\"5\":\"' + YTX_CONFIG._reqAuth + '\",\"6\":' + clientNo + '  }}'
      },
      _buildUploadAvatarReq: function (getFriendInfoBuilder, callback, onError, progress) {
        ...
        YTX_CONFIG._sendFile(getFriendInfoBuilder.getFile(), "", YTX_CONFIG._userName, 6, msgId, "", "1", callback, onError, progress, getFriendInfoBuilder.getFile().name.replace(/\s/g, ""), "", true)
      },
      ...
      _parseCallEventData: function (obj) {
        ...
        resp = new YTX_CONFIG._protobuf._CallEventData(data["1"], data["2"], data["3"], data["5"], data["7"], data["13"], data["17"], data["10"], data["14"], "", data["25"]);
        return resp
      },
    ...
    _sendFile: function (fileInfo, content, receiver, msgType, msgId, msgDomain, type, callback, onError, progress, fileName, orignMsgId, isAvatar) {
      ...
      function sendFileByStep() {
        ...
        k = 0;
        if (start < total) {
          sendStr = YTX_CONFIG._protobuf._buildSendSchedule(function () {}, function () {}, 456); //构建发送进度消息
          ws.send(sendStr);
          //设置超时保护
    ...
    GetRequestHistoryListReq: function (getRequestHistoryListBuilder, callback, onError) {
      var sendStr = YTX_CONFIG._protobuf._buildGetRequestHistoryListReq(getRequestHistoryListBuilder, callback, onError);
      if (!!sendStr) {
        YTX_CONFIG._sendMsg(sendStr)
      }
    },
    GetOtherInfoReq: function (getOtherInfoBuilder, callback, onError) {
      var sendStr = YTX_CONFIG._protobuf._buildGetOtherInfoReq(getOtherInfoBuilder, callback, onError);
      if (!!sendStr) {
        YTX_CONFIG._sendMsg(sendStr)
      }
    },
    SetUserVerifyReq: function (setUserVerifyBuilder, callback, onError) {
      var sendStr = YTX_CONFIG._protobuf._buildSetUserVerifyReq(setUserVerifyBuilder, callback, onError);
      if (!!sendStr) {
        YTX_CONFIG._sendMsg(sendStr)
      }
    },
    AgreeFriendRequestReq: function (agreeFriendRequestBuilder, callback, onError) {
      var sendStr = YTX_CONFIG._protobuf._buildAgreeFriendRequestReq(agreeFriendRequestBuilder, callback, onError);
      if (!!sendStr) {
        YTX_CONFIG._sendMsg(sendStr)
      }
    },
    ...
    RefuseFriendRequestReq: function (refuseFriendRequestBuilder, callback, onError) {
      var sendStr = YTX_CONFIG._protobuf._buildRefuseFriendRequestReq(refuseFriendRequestBuilder, callback, onError);
      if (!!sendStr) {
        YTX_CONFIG._sendMsg(sendStr)
      }
    },
    GetFriendInfoReq: function (getFriendInfoBuilder, callback, onError) {
      var sendStr = YTX_CONFIG._protobuf._buildGetFriendInfoReq(getFriendInfoBuilder, callback, onError);
      if (!!sendStr) {
        YTX_CONFIG._sendMsg(sendStr)
      }
    },
    UploadAvatarReq: function (uploadAvatarBuilder, callback, onError, process) {
      var sendStr = YTX_CONFIG._protobuf._buildUploadAvatarReq(uploadAvatarBuilder, callback, onError, process);
      if (!!sendStr) {
        YTX_CONFIG._sendMsg(sendStr)
      }
    },
    ...
    _Meeting: {
      ...
      _sendMeetingAck: function (callEventData) {
        ...
        if (("getReceivers" in pc && pc.getReceivers().length > 0) ||
      ...
      _cancelScreen: function(){
        ...
        var CallEventBuilder = new YTX_CONFIG._protobuf._CallEventData(44,
          YTX_Meeting.callId, null, YTX_CONFIG._Meeting._screenShareDate.getCalled(),
          YTX_CONFIG._userName, null, null, null, JSON.stringify(objMediaInfo), null);

        var sendStr = YTX_CONFIG._protobuf._buildCallEvent(CallEventBuilder, function(){}, function(){});
        try {
          if (!!sendStr) {
            YTX_CONFIG._sendMsg(sendStr);
          }
      ...
      _screenShare: function (screenData) {
        ...
        function screenShareSend(sdp) {
          ...
          var CallEventBuilder = new YTX_CONFIG._protobuf._CallEventData(42, YTX_Meeting.callId, null, "nconf" + screenData.getCalled(), YTX_CONFIG._userName, null, null, null, JSON.stringify(mediaInfoObj), null);
      ...
      _screenShareResp: function () {
        YTX_Webrtc.handleScreenShareAnswer(function(){}, function(){});
      }
    },
  };
...
////////
  YTX_Meeting.sendMessage = function(message){
    RL_YTX_NEW.sendMsg({
      ...
      console.error(YTX_Meeting.getTimeStamp(), "[Meeting] Send Message Fail:", err, ", Message:", message);
    });
  };
...
///////////////////////callin/////////////////////
  YTX_Meeting.recvInvite = function(content){
    ...
    console.log(YTX_Meeting.getTimeStamp(), "[Meeting] Recv Invite, caller:", YTX_Meeting.caller, 
            ", called:", YTX_Meeting.called, ", mediaType:", YTX_Meeting.mediaType);
...
//////////////////////
  ...
  YTX_Meeting.recvSubscribeMediaResponse = function(content){
    ...
    if(YTX_Meeting.direction == CDirection.CallOut){
      ...
      //for callin
      YTX_Webrtc.addUserForCallIn(parameter, YTX_Meeting.subscribeMediaSuccess, YTX_Meeting.subscribeMediaFail);
    }
  };
  ...
  YTX_Meeting.subscribeMediaFail = function(err){
    console.error(YTX_Meeting.getTimeStamp(), "[Meeting] SubscribeMedia Failed, userName:", err.userName, ", userId:", err.userId,
  ...
  YTX_Meeting.unSubscribeMedia = function(antissrc, mediaType, userId, userName){
    if(!antissrc || !mediaType || !userId || !userName){
      console.error(YTX_Meeting.getTimeStamp(), "[Meeting] unSubscribeMedia parameter err, antissrc:", antissrc, 
        ", mediaType", mediaType, ", userId:", userId, ", userName:", userName);
      return;
    }
  ...
  YTX_Meeting.handleUnSubscribeMediaTimeout = function(event){
    console.error(YTX_Meeting.getTimeStamp(), "[Meeting] UnSubscribeMedia Server Timeout, userName:", event.userName, " ,userId:", event.userId, 
        ", errName:", event.err.name, ", errMessage:", event.err.message);
  ...
  YTX_Meeting.recvUnSubscribeMediaResponse = function(content){
    ...
    if(status != "200"){
      console.error(YTX_Meeting.getTimeStamp(), "[Meeting] Recv UnSubscribeMediaResponse fail status:", status);
      err = {};
      err.userId = request.userId;
      err.userName = request.userName;
      err.name = "UnSubscribeMedia Fail ";
  ...
  YTX_Meeting.handleDelUserFailed = function(err){
    console.error(YTX_Meeting.getTimeStamp(), "[Meeting] DelUser Failed, userName:", err.userName, ", userId:", err.userId, 
        ", errName:",  err.name, ", errMessage:", err.message);
    if(YTX_Meeting.onUnSubscribeMediaFail){
  ...
  YTX_Meeting.handleDelUserSuccess = function(event){
    console.log(YTX_Meeting.getTimeStamp(), "[Meeting] DelUser Success, userName:", event.userName, ", userId:", event.userId);
...
/////////////////////////////screen share////////////////////////////
  YTX_Meeting.startScreenShare = function(onSuccess, onFail){
    ...
    function publishScreenMediaSuccess(stream){
      YTX_Webrtc.buildScreenShareOffer(stream,YTX_Meeting.cancelScreenShare, YTX_Meeting.sendPublishMediaRequest, YTX_Meeting.startScreenShareFail);
    }
  };
...
//////////////////////////////////////
  ...
  YTX_Meeting.recvUnPublishMediaResponse = function(content){
    ...
    var status = customData["status"];
    if(status != "200"){
      console.error(YTX_Meeting.getTimeStamp(), "[Meeting] Recv UnPublishMediaResponse Fail, status:", status);
      return;
    }
...
//==================================webrtc=====================================//
  ...
  YTX_Webrtc.handleRequest = function(){
    ...
    switch(YTX_Webrtc._currRequset.messageId){
      case YTX_MessageId.AddUserForCallOut:
        YTX_Webrtc.addUserForCallOut(YTX_Webrtc._currRequset,YTX_Webrtc.handleRequestSuccess, YTX_Webrtc.handleRequestFail);
        break;
      case YTX_MessageId.DelUser:
        YTX_Webrtc.delUser(YTX_Webrtc._currRequset,YTX_Webrtc.handleRequestSuccess, YTX_Webrtc.handleRequestFail); 
        break;
      case YTX_MessageId.StartScreenShare:
        YTX_Webrtc.handleScreenShareAnswer(YTX_Webrtc.handleRequestSuccess, YTX_Webrtc.handleRequestFail);
        break;
  ...
  YTX_Webrtc.onTrack = function(event){
    ...
    if(YTX_CONFIG._Meeting._meetingCallData._voipMixedAudio && event.track.kind == "audio"){
      console.log(YTX_Meeting.getTimeStamp(), "[Meeting] OnAudioTrack streamId:", eventStream.id, ", kind:", event.track.kind);
  ...
  function RL_Media() {
      this.__proto__ = {
          ...
          setCallView_v1: function (view, localView, hideView) {
            ...
            function ontrack(streamContent){
              ...
              if(track.kind == "audio"){
                ...
                YTX_CONFIG._Meeting._meetingCallData._voipMixedAudio_1.setAttribute("autoplay", "autoplay");
                YTX_CONFIG.processAVStream(YTX_CONFIG._Meeting._meetingCallData._voipMixedAudio_1, streamContent.stream);
          ...
          RequestVideoBuilder: function (memberid, mediaInfo, called, callId, caller, deviceId, callType, video, userName) {
            this._mediaInfo = mediaInfo;
            this._memberid = memberid;
            this._called = called;
            this._caller = caller;
      ...
            YTX_Meeting.setOnUnSubscribeMediaSuccess(callback);
          ...
          subscribeMedia: function(parameter, onSuccess, onFail){
            ...
            YTX_Meeting.subscribeMedia(parameter.antiSsrc, parameter.antiEncodeType, 
              parameter.mediaType, parameter.memberId, parameter.userName);
          },
      ...
            YTX_Meeting.setOnUnSubscribeMediaSuccess(onSuccess);
            YTX_Meeting.unSubscribeMedia(parameter.antissrc, parameter.mediaType, parameter.memberId, parameter.userName);
          },
