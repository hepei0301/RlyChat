...
var YTX_CONFIG = {
  ...
  _protobuf: {
    ...
    _buildSetMyInfo: function (uploadPersonInfoBuilder, callback, onError) {
      ...
      return (
        '{"MsgLite":{"1":' + YTX_CONFIG._prototype._setMyInfo + ',"2":' + uploadPersonInfoStr + ',"3":' + YTX_CONFIG._generateClientNo(callback, onError) + '}}'
      );
    },
      ...
      if (report.type == 'inbound-rtp') {
        ...
        if (!YTX_Webrtc._staticsMap.has(report.ssrc)) {
          ...
          currReport.bytesReceivedRate = parseInt(
            ((currReport.bytesReceived - lastReport.bytesReceived) / (currReport.timestamp - lastReport.timestamp)) * 8 * 1000
          );
          currReport.packetsReceivedRate = parseInt(
            ((currReport.packetsReceived - lastReport.packetsReceived) / (currReport.timestamp - lastReport.timestamp)) * 1000
          );
          currReport.headerBytesReceivedRate = parseInt(
            ((currReport.headerBytesReceived - lastReport.headerBytesReceived) / (currReport.timestamp - lastReport.timestamp)) * 8 * 1000
          );
          if (report.framesDecoded) {
            currReport.framesDecoded = report.framesDecoded;
            currReport.framesDecodedRate = parseInt(
              ((currReport.framesDecoded - lastReport.framesDecoded) / (currReport.timestamp - lastReport.timestamp)) * 1000
            );
          }
        }
...
YTX_Webrtc.handleScreenShareAnswer = function (onSuccess, onFail) {
  ...
  YTX_Webrtc._peerConnection
    ...
    .catch(function (err) {
      console.error(YTX_Meeting.getTimeStamp(), '[Meeting] ScreenShare Fail, errName', err.name, ', errMessage:', err.message);
      onFail(err);
    });
};
...
YTX_Webrtc.cancelScreenShare = function (onSuccess, onFail) {
  ...
  let fixSdp = YTX_Webrtc._peerConnection.remoteDescription.sdp;
  fixSdp = YTX_Webrtc.setVideoRateLimit(fixSdp, YTX_Webrtc.videoMaxBitrate, YTX_Webrtc.videoMinBitrate, YTX_Webrtc.videoStartBitrate);
  ...
  YTX_Webrtc._peerConnection
    ...
    .catch(function (err) {
      console.error(YTX_Meeting.getTimeStamp(), '[Meeting] cancelScreenShare createOffer failed, errName:', err.name, ', errMessage:', err.message);
      onFail(err);
    });
  /*
...
YTX_Webrtc.changeAudioInput = function (deviceId, onSuccess, onFail) {
  ...
  navigator.mediaDevices
    ...
    .catch(function (err) {
      console.error(YTX_Meeting.getTimeStamp(), '[Meeting] changeAudioInput fail, deviceId:', deviceId, ', errName:', err.name, ', errMessage:', err.message);
      onFail(err);
    });
};
...
YTX_Webrtc.changeVideoInput = function (deviceId, onSuccess, onFail) {
  ...
  navigator.mediaDevices
    ...
    .catch(function (err) {
      console.error(YTX_Meeting.getTimeStamp(), '[Meeting] changeVideoInput fail, deviceId:', deviceId, ', errName:', err.name, ', errMessage:', err.message);
      onFail(err);
    });
};
...
function RL_Media() {
  this.__proto__ = {
    ...
    subscribeMedia: function (parameter, onSuccess, onFail) {
      ...
      YTX_Meeting.subscribeMedia(parameter.antiSsrc, parameter.antiEncodeType, parameter.mediaType, parameter.memberId, parameter.userName);
    },
