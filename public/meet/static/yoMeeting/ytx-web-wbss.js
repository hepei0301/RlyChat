(function (e) {
  ...
  var YTX_CONFIG = {
    ...
    _initScoket: function (callback, onError, reset) {
      ...
      if (!YTX_CONFIG._isConnect) {
        if (!YTX_CONFIG._isConnecting) {
          var serverip;
          if (YTX_CONFIG._isSSLEncrypt) serverip = 'wss://' + Base64.decode(YTX_CONFIG._server_ip[0]);
          ...
          YTX_CONFIG._socket.onopen = function (event) {
            ...
            if (!YTX_CONFIG._isReconnect) YTX_CONFIG._log(YTX_CONFIG._logLev._INFO, 'Client connect to Server ');
            else {
    ...
    _unInitSocket: function () {
      ...
      if (!!YTX_CONFIG._failHeartBeatInterval) window.clearInterval(YTX_CONFIG._failHeartBeatInterval);
    },
    // 用户和房间数据
    ...
    _canvasApp: function () {
      ...
      function generateElementNode(eventSatus) {
        ...
        // 激光笔up状态设置无效
        if (YTX_CONFIG._drawContext.shapeType == RL_WBSS_YTX._shapeType.LASERPEN && eventSatus == MOUSE_EVENT_STATUS.EVENT_UP ? true : false) {
      ...
      function isPosRangeInView(pos) {
        if (pi) {
          ...
          if (pos.x < pi.fromX || pos.x > pi.fromX + pi.viewWidth || pos.y < pi.fromY || pos.y > pi.fromY + pi.viewHeight) {
            return false;
          }
          return true;
        } else {
        ...
        if (YTX_CONFIG._drawContext.curDoc > 0) {
          ...
          if (!pi.image) {
            ...
            log(
              ...
              'draw(' + YTX_CONFIG._drawCnt + ') ' + 'viewinfo(' + pi.fromX + ',' + pi.fromY + ',' + pi.viewWidth + ',' + pi.viewHeight + ')'
            );
          }
    ...
    checkIfReGetViewInfo: function (roomId, docId) {
      ...
      if (newWidth != YTX_CONFIG._drawContext.canvasLastWidth || newHeight != YTX_CONFIG._drawContext.canvasLastHeight) {
        log(
          YTX_CONFIG._logLev._DEBUG,
          'draw(' +
            YTX_CONFIG._drawCnt +
  ...
  window.RL_WBSS_YTX = window.RL_WBSS_YTX || {
    ...
    setLineColor: function (ColorBuilder) {
      YTX_CONFIG._drawContext.lineColor = (ColorBuilder.getR() << 24) | (ColorBuilder.getG() << 16) | (ColorBuilder.getB() << 8) | (ColorBuilder.getA() << 0);
    },
