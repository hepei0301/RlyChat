(function () {
  ...
  function RL_YTX() {
    ...
    var YTX_CONFIG = {
      ...
      // serverIp: ['d3NzOi8vdGVzdGg1LnlvbWVldGluZy5jb20vd3M='], //ws地址
      serverIp: ['d3NzOi8vaDUtaW0ueXVudG9uZ3h1bi5jb206ODA2MS93cw=='],
    ...
    var YTX_Methods = {
      ...
      onResponse: function (obj) {
        ...
        if (type === commonConfig.msgTypeNo._callRoute) {
          ...
          return;
        } else if (type === commonConfig.msgTypeNo._onLineStateNotify) {
          //上线下线消息通知
          console.log('receiveOnLineStateNotify----', obj);
          if (self.chatActive) Chat.receiveOnLineStateNotify(obj);
          return;
  ...
  function RL_Chat() {
    ...
    var chatConfig = {
      ...
      lvsServer: 'http://192.168.26.114', //图片服务器地址
      receiveMsgBuf: {},
      chatRoomId: null,
      chatRoomNickName: '',
      chatRoomInfoExt: '',
    ...
    var chatMethods = {
      ...
      buildSendTxt: function (data, callback, onError) {
        ...
        if (data.atAccounts || data.type === self.msgType._COMMAND) {
          var obj = {};
      ...
      noticeAppFn: function (obj) {
        ...
        console.log('noticeAppFn---', obj.msgId + '');
    ...
    this.__proto__ = {
      ...
      processSyncMsg: function (type, endVersion, requestType, repeat) {
        ...
        if (!!endVersion && endVersion < commonConfig.maxMsgVersion) {
          ...
          for (var i in chatConfig.receiveMsgBuf) {
            ...
            } else if (!chatConfig.receiveMsgBuf[parseInt(i) - 1]) {
              //消息暂存中不存在本地消息版本号+1的消息，中间缺消息，存下endVersion进行下次拉取
              end = parseInt(i) - 1;
            }
          }
          if (msgArr.length > 0) self.noticeApp(msgArr);
          if (commonConfig.msgVersion !== commonConfig.maxMsgVersion) {
            //若本地消息版本号不等于最大消息版本号，需要重新拉取离线消息
            chatMethods.buildSyncMessage(
              {
                startVersion: commonConfig.msgVersion + 1,
                endVersion: end,
      ...
      receivePushMsg: function (obj) {
        ...
        if (commonConfig.msgVersion % self.maxSyncNum === 0) {
          self.confirmMsg();
        }
      },
      ...
      receiveSyncMsg: function (obj, request) {
        ...
        for (var i = 0; i < resps.length; i++) {
          ...
          if (commonConfig.msgVersion >= resp.version) {
            ...
            // 当收到的消息版本号等于  本地消息版本号+1时
            commonConfig.msgVersion = resp.version > commonConfig.msgVersion ? resp.version : commonConfig.msgVersion; //更新本地版本号
            if (commonConfig.msgVersion % self.maxSyncNum === 0) {
              self.confirmMsg();
            }
    ...
    this.__proto__ = {
      ...
      StartWhiteboardSharing: function (data, callback, onError) {
        meetMethods.buildConferenceMsg(
          ...
          commonMethods.assign({ confId: self.confId }, data),
          function (e) {
            var resp = JSON.parse(e.result);
            if (resp.statusCode !== '000000') {
              onError(resp);
      ...
      leaveMeeting: function (data, callback, onError) {
        ...
        meetMethods.buildConferenceMsg('/REST/Conference/Member/Quit?source=SDK', commonMethods.assign({ confId: self.confId }, data), callback, onError);
      ...
      DeleteMeeting: function (data, callback, onError) {
        meetMethods.buildConferenceMsg('/REST/Conference/Delete?source=SDK', commonMethods.assign({ confId: self.confId }, data), callback, onError);
      ...
      UpdateAbstract: function (data, callback, onError) {
        meetMethods.buildConferenceMsg('/REST/Conference/Abstract/Update?source=SDK', commonMethods.assign({ confId: self.confId }, data), callback, onError);
      },
      //锁定会议
      LockMeet: function (data, callback, onError) {
        meetMethods.buildConferenceMsg('/REST/Conference/LockUnlock?source=SDK', commonMethods.assign({ confId: self.confId }, data), callback, onError);
      },
      //删除摘要
