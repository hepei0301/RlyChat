(function (e) {
  ...
  YTX.prototype = {
    loginCallBack: function (user_account, user_name, myUnitContactsList) {
      ...
      window.parent.RL_Chat.uploadPersonInfo(
        {
          nickName: user_name,
          sex: $('#person_sex').val(),
      ...
      //登陆后从本地获取联系人列表
      // 注册PUSH监听
      IM._onMsgReceiveListener = window.parent.RL_Chat.onMsgReceiveListener(function (obj) {
        setTimeout(function () {
          IM.EV_onMsgReceiveListener(obj);
        }, 1000);
      });
      // 注册群组通知事件监听
      IM._noticeReceiveListener = window.parent.RL_Chat.onNoticeReceiveListener(function (obj) {
    ...
    init: function () {
      ...
      window.addEventListener('message', (event) => {
        ...
        if (IM.postMessage.type === 'openDev') {
          ...
          IM.sendVoipCall1(chats, 0, false);
        }
        if (IM.postMessage.type === 'closeDev') {
          IM.cancelVoipCall();
        }
      });
      ...
      window.parent.RL_YTX_NEW.chatInit({
        fileServerIp: ip.fileServerIp,
        lvsServer: ip.lvsServer,
      });
      // window.parent.RL_YTX_NEW.meetInit({});
      if (resp.code != 200) {
        alert('SDK初始化错误');
        return;
      ...
      $('.group_list').on('click', '[data-groupid]', function () {
        ...
        if (message && IM.lastGroupId) {
          ...
          if (gid != IM.lastGroupId) {
            var info =
              message.length > 10
                ? '<em style="color:red;">[草稿]</em> ' + message.substring(0, 10) + ' ...'
                : '<em style="color:red;">[草稿]</em> ' + message.substring(0, 10);
            $('.group_list .discuss')
              .find('[data-groupid="' + IM.lastGroupId + '"]')
              .find('.draft')
              .show()
              .html(info);
          }
        } else if (!IM.hasMsg && IM.lastGroupId) {
          //删已有草稿
          var index = IM.msgGroups.findIndex(function (e, index) {
            return e.lastGroupId == IM.lastGroupId;
          });
      ...
      $(document).on('click', '[data-btn="creategroup"]', function () {
        var tag;
        var target;
        if ($(this).attr('data-cgroup-type') == 1) {
          //讨论组
          tag = $('#cDisguss');
          target = 1;
        } else {
          ...
          if (chatAim != IM.lastId) {
            var info =
              message.length > 10
                ...
                : '<em style="color:red;">[草稿]</em> ' + message.substring(0, 10);
            $('.discuss.normal_chat')
              .find('[data-im-contact="' + IM.lastId + '"]')
              .children('.draft')
              .show()
              .html(info);
          }
        } else if (!IM.hasMsg && IM.lastId) {
          //删已有草稿
          var index = IM.msgs.findIndex(function (e, index) {
            return e.lastId == IM.lastId;
          });
      ...
      $(document).on('click', "[data-voip='video']", function (e) {
        ...
        // window.parent.RL_YTX.setCallView(distance, local);
      ...
      $(document).on('click', '[data-silence]', function (e) {
        ...
        if (f == false) {
          window.parent.RL_Media.closeMicrophone();
        }
        if (f == true) {
          window.parent.RL_Media.openMicrophone();
        }
      ...
      $(document).on('click', '[data-camera]', function (e) {
        ...
        // window.parent.RL_Media.deployVideoVoice(f,"video");
        if (f == false) {
          window.parent.RL_Media.closeCamera();
        }
        if (f == true) {
          window.parent.RL_Media.openCamera();
        }
        $(this).attr('data-camera', !f);
        $(this).html((f ? '关闭' : '开启') + '摄像头');
      });

      $(document).on('click', '[data-btn="searchGroup"]', function () {
        //搜索群按钮事件
        var type = $('#searchWayValue').serializeArray();
        var keyword = $('[data-input="searchGroup"]').val();
        IM.serchGroup(type[0].value, keyword);
      });
      $(document).on('click', '[data-group-manager]', function () {
        //设为管理员
        var _this = $(this);
      ...
      $(document).on('click', '[data-group-transfer]', function (e) {
        //转让群主
        var _this = $(this);
        var groupId = $('#group').attr('data-groupid');
      ...
      $(document).on('click', '[data-group-banned]', function (e) {
        //禁言
        var _this = $(this);
        var groupId = $('#group').attr('data-groupid');
      ...
      $(document).on('click', '[data-group-kick]', function (e) {
        //踢出成员
        var _this = $(this);
        var groupId = $('#group').attr('data-groupid');
      ...
      $(document).on('blur', '[data-replace]', function (e) {
        //修改信息
        var _this = $(this);
        var val = _this.html();
        if (/[\s,<br>]/g.test(val)) {
      ...
      $(document).on('keydown', '[data-replace]', function (e) {
        //修改信息
        if (e.keyCode == 13) {
          e.preventDefault();
          $(this).blur();
      ...
      $(document).on('click', '[data-btn="invitemember"]', function (e) {
        //邀请成员
      ...
      $(document).on('click', '[data-btn="quitdiscuss"]', function (e) {
        //退出讨论组
        var gruopId = $('#group').attr('data-groupid').toString();
        IM.EV_quitGroup(gruopId, function () {
          $('#group').modal('hide');
      ...
      $(document).on('mouseenter', '[data-autodismiss="true"]', function () {
        //鼠标滑过关闭消息提示
        $(this).addClass('undock');
      });

      $(document).on('click', '[data-group-refuse]', function () {
        //拒绝群组邀请
        var _this = $(this);
        var groupId = _this.attr('data-group-refuse');
        var invitor = _this.attr('data-group-invitor').toString();
        IM.EV_processGroupInvite(
          groupId,
          invitor,
          1,
          function () {
      ...
      $(document).on('click', '[data-group-accept]', function (e) {
        //接受群组邀请
        var _this = $(this);
      ...
      $(document).on('click', '[data-group-reject]', function () {
        //拒绝群组申请
        var _this = $(this);
      ...
      $(document).on('focus', '[data-chat-type="c"] [data-chat-input="chatinput"]', function (e) {
        //正在输入状态
        IM.DO_notice(1);
      });
      $(document).on('blur', '[data-chat-type="c"] [data-chat-input="chatinput"]', function (e) {
        //正在输入状态
        IM.DO_notice(0);
      });

      $(document).on('click', '.atMsg button', function (e) {
        //@列表点击事件
      ...
      $(document).on('click', '[data-firemsg]', function (e) {
        // 阅后即焚 （将阅后即焚图标点亮的存到localStorage）
        var chartId = IM.chat_window.attr('data-chat-with');
        IM.chartIds = JSON.parse(window.localStorage.getItem('chartIds')) || [];
      ...
      $(document).on('click', '[data-msgcontent]', function (e) {
        //阅后即焚消息展示
        var _this = $(this);
      ...
      $(document).on('click', '[data-btn="joinGroup"]', function (e) {
        var $this = $(this);
        var gid = $(this).attr('data-groupid');
        window.parent.RL_Chat.joinGroup(
          {
            groupId: gid,
          },
          function (e) {
      ...
      $(document).on('click', '[data-takepic]', function (e) {
        //开启拍照功能
        console.log('拍照');
        IM.preTakePicture();
      });
      ...
      $(document).on('click', '#snapCancle', function (e) {
        //取消拍照
        console.log('取消拍照');
    ...
    EV_sendfile: function (msgId, file, type, receiver) {
      ...
      var fffff = file.name;
      var indexMsgId = window.parent.RL_Chat.sendMsg(
        objNew,
        function (e) {
          //发送成功
    ...
    DO_checkContact: function (contactVal) {
      ...
      if (contactVal.indexOf('@') > -1) {
        var regx2 =
          /^([a-zA-Z0-9]{32}#)?[a-zA-Z0-9_-]{1,}@(([a-zA-z0-9]-*){1,}.){1,3}[a-zA-z-]{1,}$/;
        ...
        if (regx1.exec(contactVal) == null) {
          IM.HTML_showAlert('alert-error', '只能是数字字母点下划线');
          return false;
        }
      }
      return true;
    ...
    EV_sendTextMsg: function (oldMsgid, text, receiver, isresend, type, ms) {
      ...
      var msgId = window.parent.RL_Chat.sendMsg(
        data,
        function (obj) {
          if (!ms) {
            return;
        ...
        function (obj) {
          setTimeout(function () {
            //断线的时候如果不延迟会出现找不到标签的情况，延迟0.3秒可解决。
            if (obj.code == 580023) {
    ...
    showGroupInfo: function (tar) {
      ...
      var groupTar = $('#group');
      $('[data-group="invite"]').tagsinput('removeAll');
      window.parent.RL_Chat.getGroupDetail(
        {
          groupId: gid,
        },
        function (obj) {
          ...
          IM.getGroupMember(gid, function (memb) {
            ...
            for (var i = 0; i < memb.length; i++) {
              if (memb[i].member == IM.user_account && memb[i].role == 2) {
                //判断是不是管理员
            ...
            if (memberRole !== 3) {
                ...
                IM.EV_setManager(
                  ...
                  function (role) {
                    for (var i = 0; i < multiArr.length; i++) {
                      $("[data-group-manager='" + multiArr[i] + "']")
                        .html('设为管理员')
                        .parent()
                        .attr('data-group-memberrule', role);
                    }
                  },
                  multiArr
                );
              ...
              $('.cancel-N').click(function () {
                IM.EV_bannedMember(
                  ...
                  function () {
                    for (var i = 0; i < multiArr.length; i++) {
                      $("[data-group-banned='" + multiArr[i] + "']")
                        .html('禁言')
                        .parent()
                        .attr('data-group-speakstate', 1);
                    }
                  },
                  multiArr
                );
            ...
            for (var i = 0; i < memb.length; i++) {
              html +=
                ...
                '</p>' +
                '<button class="kick" data-group-kick="' +
                memb[i].member +
                '">踢出</button>' +
                '<button class="banned" data-group-banned="' +
                memb[i].member +
                '" data-group-speakstate="' +
                memb[i].speakState +
                '">' +
                (memb[i].speakState == 1 ? '禁言' : '解禁') +
                '</button>' +
                '<button class="transfer" data-group-transfer="' +
                memb[i].member +
                '">转让</button>' +
                '<button class="manager" data-group-manager="' +
                memb[i].member +
                '">' +
                (memb[i].role == 2 && creator ? '取消管理员' : '设为管理员') +
                '</button>' +
                '</li>';
            }
    ...
    getGroupMember: function (gid, callback) {
      window.parent.RL_Chat.getGroupMemberList(
        {
          groupId: gid,
        },
        function (obj) {
    ...
    DO_logout: function (needLogout) {
      ...
      window.parent.RL_YTX_NEW.logout(
        function () {},
        function (err) {
          console.log(err);
        }
    ...
    EV_login: function (user_account, pwd, sig, timestamp) {
      ...
      window.parent.RL_YTX_NEW.login(
        data,
        function (obj) {
          IM.user_account = user_account;
          IM.is_online = true;
          ...
          $('#userAcc').val(user_account);
          IM.setContactMember();
          // window.parent.RL_AV.init({
          //   mwsgServer: ip.mwsgServer
          // });
          // window.parent.RL_Chat.uploadUserDeviceDetail({
          //   appVersion: '5.4.0',
          //   ip: returnCitySN["cip"],
    ...
    EV_onMsgReceiveListener: function (obj) {
      ...
      if (b_isGroupMsg) {
        ...
        if (obj.msgType === 2) {
          window.parent.RL_Chat.getAudioSource(
            obj.msgFileUrl,
            function (res) {
        ...
        if (obj.msgType == 6) {
          msgDiv['msgFileName'] = obj.msgFileName;
          msgDiv['msgFileSize'] = obj.msgFileSize;
          window.parent.RL_Chat.getFileSource(
            obj.msgFileUrl,
            function (e) {
              console.log('解压缩附件完成');
    ...
    EV_getPersonalInfo: function () {
      ...
      window.parent.RL_Chat.getMyInfo(
        function (obj) {
    ...
    msgAlert: function (noticeMsg, autoDismiss, groupId, invitor, name, acc, ref) {
      var html =
        '<div class="audioAline" data-autodismiss="' +
        autoDismiss +
        '">' +
        '<div class="imgDiv">' +
    ...
    DO_deskNotice: function (
      ...
      var instance = new IM._Notification(title, {
        body: body,
    ...
    EV_getGroupList: function () {
      ...
      window.parent.RL_Chat.getGroupList(
        {
          target: 125,
          pageSize: -1,
        },
    ...
    addMsgToChatWindow: function (obj) {
      ...
      if (obj.msgType == 1) {
        ...
        codes =
          ...
          obj.msgFileName +
          '"><img src="img/filepic.png" style="width:20px;cursor: auto;"/>' +
          obj.msgFileName +
          ' : 文件大小：' +
          obj.msgFileSize +
          'kb</a>';
    ...
    HTML_addChatToList: function (contactId, contactVal, content_type, multi, show) {
      ...
      switch (content_type) {
        case 'C':
          ...
          if (msg) {
            msgDiv =
              '<li class="contactList"  data-im-contact = ' +
              contactId +
              ' data-user-state="' +
              (!!multi ? multi.state : 2) +
              '"><i class="icon_header"></i>' +
              '<span class="discuss_name">' +
              contactVal +
              '</span>' +
              '<em class="draft"><em style="color:red;">[草稿]</em>' +
              msg.message +
              '</em>' +
              '<button class="removeCon" data-btn="removeCon">删除</button></li>';
          } else {
            msgDiv =
              '<li class="contactList"  data-im-contact = ' +
              contactId +
        ...
        case 'G':
          msgDiv =
            '<li class="groupChat" data-groupname="' +
            contactVal +
            '"  data-groupid="' +
            contactId +
            '" data-isnotice="' +
        ...
        case 'D':
          msgDiv =
            ...
            '" data-isnotice="' +
            multi.isNotice +
            '"><img src="img/head_portrait/discussion_header/discussio_groups_01.png"> ' +
            '<div> <span class="discuss_name">' +
            contactVal +
            '</span><em class="draft"></em></div></li>';
          $('.group_list').find('.discuss').prepend(msgDiv);
          break;
        case 'M':
          break;
    ...
    serchGroup: function (type, keyWords) {
      window.parent.RL_Chat.searchGroups(
        {
          ...
          for (var i = 0; i < obj.length; i++) {
            if (IM.group_list[obj[i].groupId]) {
              ...
              html +=
                '<li> <img src="img/head_portrait/headerX40.png"> <p class="cg_name">' +
                obj[i].name +
                '</p><button class="join" data-btn="joinGroup" data-groupId="' +
                obj[i].groupId +
                '">加入</button></li>';
            }
          }
    ...
    createAudioView: function (obj, isSender, type) {
      ...
      var part1 =
        ...
        obj +
        ' data-call-state="1" data-call-type=' +
        type +
        '>' +
        '<div class="imgDiv">' +
      ...
      if (!!isSender) {
        part2 =
          '<span class="color6060" data-call="msg" >等待对方回应</span>' +
        ...
        part2 =
          '<span class="color6060" data-call="msg" >对方邀请你' +
          (type == 1 ? '视频' : '语音') +
          '通话</span>' +
          '<audio autoplay="autoplay" id="audioCall"></audio>' +
          '<div class="audioBtn">' +
      ...
      // window.parent.RL_Media.setCallView_v1(document.getElementById('audioCall'), null);

      window.parent.RL_Media.setOnTrack(ontrack);
    ...
    sendVoipCall: function (calledUser, callType, tel, nickName, deviceId) {
      ...
      window.parent.RL_Media.makeCall(
        {
          caller: window.parent.RL_YTX_NEW.userName,
    ...
    acceptCall: function (deviceId) {
      ...
      if (t == 1) {
        ...
        // window.parent.RL_Media.setCallView_v1(distance, local);

        window.parent.RL_Media.setOnPublishMedia(publishMedia);
        window.parent.RL_Media.setOnTrack(ontrack);
        ...
        window.parent.RL_Media.acceptCall(
          function (e) {
            console.log(e);
    ...
    refuseCall: function (e) {
      IM.isCalling = false;
      document.getElementById('call_ring').pause();
      window.parent.RL_Media.rejectCall(
        function (e) {},
        function (e) {
          $.scojs_message(e.code + ' : ' + e.msg, $.scojs_message.TYPE_ERROR);
        }
    ...
    crateGroup: function (obj, callback) {
      window.parent.RL_Chat.createGroup(
        {
    ...
    EV_setManager: function (groupid, memberId, role, callback, member) {
      window.parent.RL_Chat.setGroupMemberRole(
        {
          groupId: groupid,
          memberId: memberId,
          member: member,
    ...
    EV_kickOutMember: function (groupId, memberId, callback) {
      //踢人
      window.parent.RL_Chat.deleteGroupMember(
        {
          groupId: groupId,
          memberId: [memberId + ''],
        },
    ...
    EV_transferGroup: function (groupid, memberId, callback) {
      //转让
      window.parent.RL_Chat.setGroupMemberRole(
        {
          role: 1,
    ...
    EV_bannedMember: function (groupId, memberId, isBanned, callback, member) {
      //禁言
      window.parent.RL_Chat.forbidMemberSpeak(
        {
          groupId: groupId,
          memberId: memberId,
    ...
    EV_dismissGroup: function (groupId, callback) {
      //解散群组
      window.parent.RL_Chat.dismissGroup(
        {
          groupId: groupId,
        },
        function (e) {
    ...
    EV_quitGroup: function (groupId, callback) {
      window.parent.RL_Chat.quitGroup(
        {
          groupId: groupId,
        },
        function (e) {
    ...
    EV_getUserState: function () {
      ...
      window.parent.RL_Chat.getUserState(
        {
          newUserstate: true,
          useracc: arr,
        },
    ...
    EV_setGroupMemberNick: function (groupId, memberId, nick, callback) {
      window.parent.RL_Chat.modifyMemberCard(
        {
          memberId: memberId,
          belong: groupId,
          display: nick,
    ...
    EV_inviteMember: function (groupId, members, needConfirm, callback) {
      window.parent.RL_Chat.inviteJoinGroup(
        {
          groupId: groupId,
          members: members,
    ...
    EV_processGroupInvite: function (groupId, inviter, val, callback, onerror) {
      window.parent.RL_Chat.confirmInviteJoinGroup(
        {
          inviter: inviter,
          groupId: groupId,
          confirm: val,
    ...
    EV_confirmInviteJoinGroup: function (groupId, membrer, confirm, callback, onerror) {
      window.parent.RL_Chat.confirmJoinGroup(
        {
          memberId: membrer,
          groupId: groupId,
          confirm: confirm,
    ...
    EV_setGroupName: function (groupId, groupName, callback) {
      window.parent.RL_Chat.modifyGroup(
        {
          groupId: groupId,
          groupName: groupName,
        },
    ...
    choseEmoji: function (unified, unicode) {
      ...
      var content_emoji =
        '<img imtype="content_emoji" emoji_value_unicode="' +
        unicode +
    ...
    EV_msgBack: function (msgId, callback) {
      window.parent.RL_Chat.msgOperation(
        {
          msgId: msgId,
          type: 1,
        },
    ...
    preTakePicture: function () {
      ...
      obj.tag = video;
      window.parent.RL_Media.photo.apply(
        obj,
        function (e) {},
        function (err) {
          console.log(err);
    ...
    cancleTakePicture: function () {
      window.parent.RL_Media.photo.cancel();
      $('#takePic').hide();
    },
    takePicture: function () {
      var resultObj = window.parent.RL_Media.photo.make();
      //拍照成功
    ...
    EV_setGroupNotice: function (groupId, isNotice, callback) {
      window.parent.RL_Chat.setGroupMessageRule(
        {
          groupId: groupId,
          isNotice: isNotice,
        },
    ...
    EV_deleteReadMsg: function (msgId, callback) {
      window.parent.RL_Chat.deleteReadMsg(
        {
          msgId: msgId,
        },
        function () {
    ...
    EV_SetTopContact: function (user, callback) {
      if (!user) return;
      window.parent.RL_Chat.setTopContact(
        {
          contact: user + '',
          type: 1,
        },
    ...
    EV_GetTopContact: function (callback) {
      window.parent.RL_Chat.getTopContact(
        function (e) {
          try {
    ...
    send: function () {
      window.parent.RL_MEET.ConferenceMsg(
        {
          path: '/REST/Conference/Room/List?source=SDK',
          content: {
            appid: this._appid,
            userId: this._userAcc,
            memberIdType: 1,
          },
        },
        function (e) {
