...
!(function (a) {
  ...
  function b(b, c) {
    (this.itemsArray = []),
      ...
      (this.$input = a('<input type="text" placeholder="' + this.placeholderText + '"/>').appendTo(
        this.$container
      )),
          ...
          if (!m || f.options.allowDuplicates) {
            if (!(f.items().toString().length + b.length + 1 > f.options.maxInputLength)) {
              ...
              if ((f.$element.trigger(n), !n.cancel)) {
                ...
                if (
                  (o.data('item', b),
                  f.findInputWrapper().before(o),
                  o.after(' '),
                  f.isSelect && !a('option[value="' + encodeURIComponent(i) + '"]', f.$element)[0])
                ) {
        ...
        e.$container.on(
          ...
          a.proxy(function (b) {
            ...
            e.options.freeInput &&
              ...
              (e.add(f ? d.substr(0, e.options.maxChars) : d), c.val(''), b.preventDefault());
            {
              var h = c.val().length;
              Math.ceil(h / 5);
            }
