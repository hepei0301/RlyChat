(function () {
  ...
  var commonMethods = {
    ...
    setlocalStorage: function (version) {
      ...
      window.localStorage[commonConfig.userAcc] = version;
    },
    getlocalStorage: function () {
      ...
      var s = null;
      if ((s = window.localStorage[commonConfig.userAcc]) === undefined) {
        window.localStorage[commonConfig.userAcc] = 0;
        return '0';
      }
      console.log('getlocalStorage---', s);
      return s;
    },
  };
  ...
  function RL_YTX() {
    ...
    var YTX_CONFIG = {
      ...
      // serverIp: ['d3NzOi8vdGVzdHloaDUueW9tZWV0aW5nLmNvbTo0NDMvd3M='], //testyh
      // serverIp: ['d3NzOi8vdGVzdGg1LnlvbWVldGluZy5jb206NDQzL3dz'], //testwx
      serverIp: ['d3NzOi8vaDUtaW0ueXVudG9uZ3h1bi5jb206ODA2MS93cw=='], //容信
    ...
    var YTX_Methods = {
      ...
      onRequestResp: function (obj) {
        ...
        if (self.chatActive) Chat.receiveRequestResponse(obj, callback);
      ...
      onResponse: function (obj) {
        ...
        if (!obj['MsgLite']) {
          ...
          } else if (!!obj['resp'] && self.chatActive) {
        ...
        if (type === commonConfig.msgTypeNo._callRoute) {
          ...
          //TODO 会议相关，会议有标识 ?
          if (!!obj['2'] && ROOT.RL_AV.isActive) {
            ROOT.RL_Media.processMsg(obj);
          }
          return;
        } else if (type === commonConfig.msgTypeNo._pushMsg) {
          //普通文本消息和附件走这个分支
          console.log('receivePushMsg----', obj);
          if (self.chatActive) Chat.receivePushMsg(obj);
          return;
        } else if (type === commonConfig.msgTypeNo._pushMsgNotify) {
          if (self.chatActive) Chat.receivePushMsgNotify(obj);
          return;
        } else if (type === commonConfig.msgTypeNo._syncMsg) {
          ...
          if (self.chatActive) Chat.receiveSyncMsg(obj, request);
          ...
          if (self.chatActive) Chat.receiveChatRoomMsg(obj);
          return;
        } else if (type === commonConfig.msgTypeNo._ConferenceNotice) {
          //会议通知
          if (self.meetActive) Meet.receiveConferenceNotice(obj);
          return;
        }
        var request = YTX_CONFIG.clientMap[obj['3']];
        if (!request) {
        ...
        if (type === commonConfig.msgTypeNo._login) {
          ...
          if (commonConfig.msgVersion < commonConfig.maxMsgVersion && self.chatActive) {
          ...
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseSendMsgResp');
        } else if (
          type === commonConfig.msgTypeNo._getMyInfo ||
          type == commonConfig.msgTypeNo._setMyInfo
        ) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseGetMyInfo');
        } else if (type === commonConfig.msgTypeNo._createGroup) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseCreateGroupResp');
        } else if (type === commonConfig.msgTypeNo._getOwnGroups) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseGetGroupListResp');
        } else if (type === commonConfig.msgTypeNo._queryGroupMembers) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseGetGroupMemberListResp');
        } else if (type === commonConfig.msgTypeNo._getGroupDetail) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseGetGroupDetailResp');
        } else if (type === commonConfig.msgTypeNo._searchGroups) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseSearchGroupsResp');
        } else if (type === commonConfig.msgTypeNo._queryGroupMemberCard) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseQueryGroupMemberCard');
        } else if (type === commonConfig.msgTypeNo._getUserState) {
          if (commonConfig.newUserState) {
            if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseGetUserState_multy');
          } else {
            if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseGetUserState');
          }
        } else if (type === commonConfig.msgTypeNo._chatRoom._enterChatRoom) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseEnterChatRoomResp');
        } else if (type === commonConfig.msgTypeNo._chatRoom._fetchChatroomInfo) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseFetchChatRoomInfoResp');
        } else if (type === commonConfig.msgTypeNo._chatRoom._fetchChatroomMembers) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseFetchChatRoomMembersResp');
        } else if (type === commonConfig.msgTypeNo._chatRoom.MSG_SEND_LiveChatRoom_MESSAGE) {
          if (!obj['2']) {
            if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseSendChatRoomMsgResp');
          } else {
            if (self.chatActive) Chat.receiveChatRoomMsg(obj);
          }
          return;
        } else if (type === commonConfig.msgTypeNo._getTopContact) {
          if (self.chatActive) Chat.receiveMsgResp(obj, request, 'parseGetTopContactResp');
        } else if (type === commonConfig.msgTypeNo._ConferenceMessage) {
          if (self.meetActive) Meet.receiveMsgResp(obj, request, 'parseConferenceMessageResp');
        } else {
          callback(commonMethods.parseCodeResp(obj));
        }
      ...
      onResponseErr: function (obj) {
        ...
        if (
          ...
          onError(commonMethods.parseCodeResp(obj));
          YTX_Methods.connectStateChange(5, 'connect fail,please relogin');
          return;
        }
        if (obj['1'] === commonConfig.msgTypeNo._login) {
          YTX_CONFIG.loginStatus = YTX_CONFIG.loginStatus.NoLogin;
        }
        ...
        if (obj['6'] === commonConfig.reqErrCode.MsgVersionOverdue) {
          if (!request.repeat || request.repeat < 0) {
            ...
            if (request.endVersion - commonConfig.msgVersion > 10) {
              commonConfig.msgVersion = commonConfig.msgVersion + 10;
            } else {
              commonConfig.msgVersion = request.endVersion;
            }
            if (self.chatActive) Chat.processSyncMsg();
          }
        } else {
          onError(commonMethods.parseCodeResp(obj));
        }
      ...
      generateImei: function () {
        commonConfig.imei = hex_md5(
          commonConfig.appId + commonConfig.userName + YTX_CONFIG.sdkName
        );
      },
      ...
      reconnectFn: function (callback) {
        ...
        if (YTX_CONFIG.reconnectNum > YTX_CONFIG.maxReconnectNum) {
          ...
          if (YTX_CONFIG.isOffLine) {
            ...
            YTX_CONFIG.reconnectNum = 0;
            YTX_CONFIG.reconnectInterval && clearInterval(YTX_CONFIG.reconnectInterval);
            YTX_Methods.connectStateChange(5, '超出最大重连次数，请重新登录');
          }
          return;
        }
      ...
      buildLogin: function (data, callback, onError) {
        ...
        if (data.type !== YTX_CONFIG.loginTypeNo.LoginByOther) {
          ...
          sendObj[5] = YTX_CONFIG.deviceType;
          sendObj[6] = data.sig || '';
          sendObj[7] = self.version;
      ...
      msgBuild: function (data, callback, onError) {
        ...
        var msgLite = {
          [data.MsgLiteKey]: data.MsgLiteObj,
        };
      ...
      getServerIp: function (data, callback, onError) {
        ...
        YTX_Methods.jsonp(
          ...
          function (e) {
            ...
            var server =
              'wss://' + e['webSocketServer'][0].host + ':' + e['webSocketServer'][0].port + '/ws';
            // if (self.chatActive) Chat.setConfig({
      ...
      initScoket: function (data, callback, onError) {
        ...
        console.log('data---', data, YTX_CONFIG.reconnectNum);
        if (
          data.type === undefined ||
        ...
        if (!YTX_CONFIG.isConnect) {
          if (!YTX_CONFIG.isConnecting) {
            ...
            YTX_CONFIG.socket.onclose = function (event) {
              ...
              commonMethods.logger.info(
                'Client notified socket has closed:' + JSON.stringify(event)
              );
              if (YTX_CONFIG.loginStatus === YTX_CONFIG.loginStatusNo.Logined) {
                YTX_CONFIG.isConnect = false;
                YTX_CONFIG.isConnecting = false;
      ...
      connectStateChange: function (code, msg) {
        var resp = {};
        resp.code = code;
        resp.msg = msg;
    ...
    this.__proto__ = {
      ...
      login: function (data, callback, onError) {
        ...
        if (
          data.type &&
          data.type !== YTX_CONFIG.loginTypeNo.LoginByVoip &&
          data.type !== YTX_CONFIG.loginTypeNo.LoginByOther &&
          data.type !== YTX_CONFIG.loginTypeNo.LoginByUserName
        ) {
          var resp = {};
        ...
        ROOT.commonConfig.userName = commonConfig.userName;
        commonConfig.userAcc = commonConfig.appId + '#' + data.userName;
        if (data.authSig) {
      ...
      logout: function (callback, onError) {
        ...
        if (self.chatActive) Chat.unLoad();
        if (self.meetActive) Meet.unLoad();
        callback && callback();
      },
  ...
  function RL_Chat() {
    ...
    this.maxSyncNum = 1000;
    var chatConfig = {
      // fileServerIp: "http://192.168.26.114:8090", //文件服务器地址
      //fileServerIp: "http://192.168.27.211:8090", //文件服务器地址
      fileServerIp: 'http://139.199.128.158:8090', //文件服务器地址
      // fileServerIp: "http://192.168.27.220:80", //文件服务器地址
      lvsServer: 'http://139.199.128.158', //图片服务器地址
      receiveMsgBuf: {},
      chatRoomId: null,
      chatRoomNickName: '',
      chatRoomInfoExt: '',
    ...
    var chatMethods = {
        ...
        for (var i in data) {
          ...
          if (orign.msgType === commonConfig.msgTypeNo._msg_CMD) {
            ...
            if (1 === auditType) {
              ...
              you_msgContent =
                '用户[' + people + ']成为' + groupTarget + '[' + groupName + ']管理员';
            } else {
      ...
      parseGetGroupListResp: function (obj, callback, onErr) {
        ...
        if (data.length === chatConfig.groupConfig._builder.pageSize) {
          ...
          self.getGroupList(getGroupListData, callback, onErr);
        } else {
          callback(chatConfig.groupConfig._groupArray);
          chatConfig.groupConfig._groupArray = [];
        }
      ...
      parseGetGroupMemberListResp: function (obj, callback, onErr) {
        ...
        if (data.length === chatConfig.groupConfig._builder.pageSize) {
          ...
          self.getGroupMemberList(groupMemberListData, callback, onErr);
        } else {
          callback(chatConfig.groupConfig._groupMemberArray);
          chatConfig.groupConfig._groupMemberArray = [];
        }
      ...
      buildSyncMessage: function (data, onError) {
        ...
        if (
          isNaN(data.startVersion) ||
          isNaN(data.endVersion) ||
        ...
        } else if (data.endVersion - data.startVersion <= self.maxSyncNum) {
          for (var g = data.startVersion; g < data.endVersion + 1; g++) {
            sendJsonStr['1'] += g + ',';
          }
          sendJsonStr['1'] = sendJsonStr['1'].substring(0, sendJsonStr['1'].length - 1);
        } else {
          for (var g = data.startVersion; g < parseInt(data.startVersion) + self.maxSyncNum; g++) {
            sendJsonStr['1'] += g + ',';
          }
          sendJsonStr['1'] = sendJsonStr['1'].substring(0, sendJsonStr['1'].length - 1);
        }
      ...
      buildSendFile: function (data, callback, onError) {
        ...
        var ytx_params = JSON.stringify({
          appId: commonConfig.appId,
          // appId: 'c1b2e379fc6111e8b57fecebb89e8a08',
          msgType: data.type,
      ...
      buildSendTxt: function (data, callback, onError) {
        ...
        if (data.atAccounts) {
          console.log('sendMsg:extopts=' + data.atAccounts.join(','));
          sendObj['11'] = Base64.encode(
            JSON.stringify({
              at: atAccounts,
            })
          );
        ...
        //   type: "zip",
        //   url: "http://139.199.128.158/56001/e750de49d8254db1a3d735c6c4d80b52/2019-08-09/11-24/1565321080481778816.zip",
        //   // url: "http://139.199.128.158//56001/e750de49d8254db1a3d735c6c4d80b52/2019-08-12/16-32/1565598742526562764.amr",
        // }, function (res) {
        //   console.log(res)
        // }, function (err) {
        //   console.log(err)
      ...
      buildModifyMemberCard: function (data, callback, onError) {
        ...
        if (!data.memberId || !data.belong) {
          var resp = {};
          resp.code = commonConfig.errCode._NO_REQUIRED_PARAM;
          resp.msg = funName + 'param memberId or belong is empty.';
          onError(resp);
          return;
        }
        var sendObj = {
      ...
      buildSetGroupMemberRole: function (data, callback, onError) {
        ...
        if (!data.groupId || !data.memberId || !data.role) {
          var resp = {};
          resp.code = commonConfig.errCode._NO_REQUIRED_PARAM;
          resp.msg = funName + 'param groupId or memberId or role is empty.';
          onError(resp);
          return;
        }
        RL_YTX.sendMsg(
        ...
        RL_YTX.sendMsg(
          {
            ...
            MsgLiteObj: {
              1: commonConfig.msgTypeNo._enterChatRoom,
            },
          },
          callback,
          onError
        ...
        RL_YTX.sendMsg(
          {
            ...
            msgKey: 'ExitChatroom',
            MsgLiteObj: {
              1: commonConfig.msgTypeNo._exitChatRoom,
            },
          },
          callback,
          onError
        ...
        RL_YTX.sendMsg(
          {
            ...
            MsgLiteObj: {
              1: commonConfig.msgTypeNo._fetchChatroomInfo,
            },
        ...
        RL_YTX.sendMsg(
          {
            ...
            MsgLiteObj: {
              1: commonConfig.msgTypeNo._fetchChatroomMembers,
            },
        ...
        RL_YTX.sendMsg(
          {
            sendObj,
            MsgLiteObj: {
              1: commonConfig.msgTypeNo._updateChatroomInfo,
            },
        ...
        RL_YTX.sendMsg(
          {
            sendObj,
            MsgLiteObj: {
              1: commonConfig.msgTypeNo._updateChatRoomMemberRole,
            },
      ...
      noticeAppFn(obj) {
        ...
        if (obj.msgType === 25 && obj.msgDomain) {
          try {
            let msgDomain = JSON.parse(obj.msgDomain);
            if (msgDomain.deviceno !== commonConfig.imei) {
              msgDomain.msgId = msgDomain.dateCreated + '|' + msgDomain.version;
              obj.msgDomain = JSON.stringify(msgDomain);
            }
          } catch (e) {}
        }
        if (obj.msgType === commonConfig.msgTypeNo._msg_CMD) {
          //cmd消息
        ...
        } else if (obj.msgType === commonConfig.msgTypeNo._msg_DEL) {
          //阅后即焚删除消息
        ...
        } else if (obj.msgType === commonConfig.msgTypeNo._chatRoom.ChatRoom_MESSAGE) {
          //聊天室推送消息
        ...
        } else if (obj.msgType === commonConfig.msgTypeNo.friendMsg) {
          //好友消息
          return obj;
        } else {
    ...
    this.__proto__ = {
      ...
      setConfig: function (val) {
        ...
        self.maxFileLen = chatConfig.maxFileLen;
      ...
      sendMsg: function (data, callback, onError, progress) {
        ...
        for (var i in self.msgType) {
          if (data.type === self.msgType[i]) {
            isAvailable = true;
            break;
          }
        }
      ...
      allMuteMode: function (data, callback, onError) {
        data = commonMethods.getParam(data);
        self.updateChatroomInfo(data, callback, onError);
      }, //可直接调用updateChatroomInfo方法
      defriendMember: function (data, callback, onError) {
        data = commonMethods.getParam(data);
        self.updateMemberOption(data, callback, onError);
      ...
      processSyncMsg: function (type, endVersion, requestType, repeat) {
        ...
        if (!!endVersion && endVersion < commonConfig.maxMsgVersion) {
        ...
        } else if (commonConfig.msgVersion < commonConfig.maxMsgVersion) {
          ...
          if (msgArr.length > 0) self.noticeApp(msgArr);
        ...
        function reGet(err) {
          if (err.code === 171137) {
            commonConfig.syncMsgPorcessing = false;
            self.processSyncMsg(type, endVersion, requestType, repeat);
          } else if (err.fail) {
            commonConfig.syncMsgPorcessing = false;
          }
        }
      ...
      noticeApp: function (obj) {
        ...
        for (let val in msgObj) {
          if (parseInt(val) === commonConfig.msgTypeNo._msg_CMD && chatMethods._noticeListener) {
            //cmd消息
          ...
          } else if (parseInt(val) === commonConfig.msgTypeNo.friendMsg) {
            //好友消息
            chatMethods._friendNoticeMsgListener(msgObj[val]);
          } else {
            if (!!chatMethods._pushListener) {
              chatMethods._pushListener(msgObj[val]);
              console.log('_pushListener:', msgObj[val].msgContent);
            }
          }
        }
      },
      ...
      receivePushMsg: function (obj) {
        ...
        if (pushs['1'] && commonMethods.hasLocalStorage) {
          console.log(pushs['1']);
        ...
        if (commonConfig.msgVersion + 1 === pushMsg.version) {
          ...
          commonConfig.maxMsgVersion =
            commonConfig.maxMsgVersion < pushMsg.version
              ...
              : commonConfig.maxMsgVersion;
          chatConfig.receiveMsgBuf[pushMsg.version] = pushMsg;
          self.processSyncMsg(1);
        }
        if (commonConfig.msgVersion % 10 === 0) {
          self.confirmMsg();
        }
      },
      ...
      receiveSyncMsg: function (obj, request) {
        ...
        for (var i = 0; i < resps.length; i++) {
          ...
          if (commonConfig.msgVersion >= resp.version) {
            if (!chatConfig.getMsgByArray) {
              self.noticeApp(resp);
            } else {
              msgArray.push(resp);
            }
          } else if (commonConfig.msgVersion + 1 === resp.version) {
            commonConfig.msgVersion =
              resp.version > commonConfig.msgVersion ? resp.version : commonConfig.msgVersion;
            if (commonConfig.msgVersion % 10 == 0) {
              self.confirmMsg();
            }
            if (chatConfig.receiveMsgBuf[resp.version]) {
            ...
            chatConfig.receiveMsgBuf[resp.version] = resp;
          }
        }
        if (chatConfig.getMsgByArray && msgArray.length > 0) self.noticeApp(msgArray);
        if (!continues) {
          commonConfig.syncMsgPorcessing = false;
          self.processSyncMsg(2, end, request.type);
        } else {
          commonConfig.syncMsgPorcessing = false;
          self.processSyncMsg(2);
        }
      },
      receiveChatRoomMsg: function (obj) {
        var chatRoomMsg = chatMethods.parseChatRoomMsg(obj);
        self.noticeApp(chatRoomMsg);
      },
      ...
      enterRoom: function () {
        if (chatConfig.chatRoomId) {
          self.enterChatroom(
            {
              roomId: chatConfig.chatRoomId,
              nickName: chatConfig.chatRoomNickName,
              infoExt: chatConfig.chatRoomInfoExt,
            },
            function () {},
            function () {}
          );
  ...
  function RL_MEET() {
    ...
    this.__proto__ = {
      ...
      // 加入会议
      JoinMeetRoom(data, callback, onError) {
      ...
      // 白板共享
      StartWhiteboardSharing(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      // 停止白板共享
      StopWhiteboardSharing(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      // 会议更新
      ConferenceUpdate(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //会议室列表
      RoomList(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      // 获取成员列表
      GetMeetMemberList(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      // 获取会议列表
      GetOrderMeetList(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      // 发布语音
      StartPublishVoice(data, callback, onError) {
      ...
      // 停止发布语音
      StopPublishVoice(data, callback, onError) {
      ...
      //  发布视频
      StartPublishVideo(data, callback, onError) {
      ...
      //  停止发布视频
      StopPublishVideo(data, callback, onError) {
      ...
      // 邀请
      InviteMember(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      // 控制媒体
      MediaControl(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //更新成员信息
      UpdateMeetMemberNick(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //踢出成员
      KickMeetMember(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //拒绝邀请
      RejectInvitation(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //接受邀请
      AcceptInvitation(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //退出会议
      leaveMeeting(data, callback, onError) {
      ...
      // 删除会议、结束会议
      DeleteMeeting(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //获取会议摘要列表
      GetAbstractList(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //更新会议摘要
      UpdateAbstract(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //锁定会议
      LockMeet(data, callback, onError) {
        meetMethods.buildConferenceMsg(
      ...
      //删除摘要
      DeleteAbstract(data, callback, onError) {
        meetMethods.buildConferenceMsg(
