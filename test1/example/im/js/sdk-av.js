(function () {
  ...
  function RL_AV() {
    var avConfig = {
      mwsgServer: 'wss://192.168.27.239:1443/h5websocket',
      // mwsgServer: "wss://testh5.yomeeting.com:1443/h5websocket",
      newMediaWS: null,
    ...
    this.__proto__ = {
      ...
      unload: function () {
        try {
          ...
          avConfig.newMediaWS = null;
          if (avConfig.newMediaTar) clearTimeout(avConfig.newMediaTar);
  ...
  function RL_Media() {
    ...
    var mediaMethods = {
      ...
      parseMeetingVoip: function (callEventData) {
        ...
        if (callEventData.callEvent === 1) {
          ...
          if (!!callEventData.strSDP && commonConfig.peerConnection) {
            Webrtc.setRemoteDesc(
              callEventData.strSDP,
              'answer',
              function () {},
              function (err) {
      ...
      processVoip: function (callEventData) {
        ...
        if (callEventData.callEvent === 1) {
          if (!mediaConfig.voipCallData.callEventCallId) {
            ...
            var code =
              !Webrtc.getUserMedia() || !Webrtc.getPeerConnection()
                ? commonConfig.errCode._NOT_SUPPORT_CALL
                : 200;
            mediaConfig.voipCallData.voipCallType = type;
            mediaMethods._voipListener(
              commonMethods.assign(listenData, {
                msg: 'calling',
                callType: type,
                state: 6,
                code: code,
      ...
      sendAck: function (callEventData) {
        ...
        RL_AV.newMediaConnect(function () {
          mediaMethods.newMeidaReceiveSDP(
            ...
            function (obj) {
              Webrtc.setRemoteDesc(
                mediaMethods.processSDP(obj.sdp),
                'answer',
                function () {},
                function (err) {
      ...
      buildCallEvent: function (callEventData, callback, onError) {
        ...
        if (isVoipCallArr.indexOf(parseInt(callEventData.isVoipCall)) > -1)
          sendObj[3] = callEventData.isVoipCall;
      ...
      sendOffer: function (callEventData, callback, onError) {
        ...
        console.log(pc);
        if (
          (pc.iceGatheringState === 'complete' || mediaConfig.sendOfferCount > 10) &&
      ...
      sendOfferFn: function (desc, reset, callback, onError) {
        ...
        if (
          (commonConfig.peerConnection.iceGatheringState === 'complete' ||
            mediaConfig.mediaCount > 10) &&
          mediaConfig.mediaCount > 0
        ) {
      ...
      sendMeetOffer: function (callEventData, callback, onError) {
        ...
        var pc = commonConfig.peerConnection;
        if (
          (pc.iceGatheringState === 'complete' && mediaConfig.sendMeetOfferCount > 0) ||
          mediaConfig.sendMeetOfferCount === 10
        ) {
      ...
      sendMeetAnswer: function (memberid, uuid) {
        ...
        var pc = commonConfig.peerConnection;
        if (
          (pc.iceGatheringState === 'complete' && mediaConfig.sendMeetAnswerCount > 0) ||
          mediaConfig.sendMeetAnswerCount === 10
        ) {
        ...
        try {
          navigator.mediaDevices
            ...
            .then(function (stream) {
              ...
              if (
                !mediaConfig.voipCallData.needVideo &&
                !('getTracks' in stream && mediaConfig.videoConfig.type === 2)
              ) {
        ...
        RL_AV.sendNewMediaMsg(
          {
            method: 'ReqVideo',
            memberid:
              memberid.indexOf('@') > -1 ? memberid.substr(0, memberid.indexOf('@')) : memberid,
    ...
    this.__proto__ = {
      ...
      releaseCall: function (data, callback, onError) {
        ...
        if (data.caller === commonConfig.userName && !mediaConfig.voipCallData.connected) {
          ...
          mediaConfig.voipCallData.called === commonConfig.userName &&
          !mediaConfig.voipCallData.connected
        ) {
          self.rejectCall(
            {
        ...
        mediaMethods.buildCallEvent(
          commonMethods.assign(data, { callEvent: callEventType }),
          function () {
              ...
              switch (error.code || error.name) {
                ...
                default:
                  commonMethods.logger.info(
                    '无法打开音视频。异常信息:' + (error.code || error.name)
                  );
          ...
          mediaMethods.getUserMediaPermission(
            ...
            function (error) {
              ...
              switch (error.code || error.name) {
                ...
                default:
                  commonMethods.logger.info(
                    'can`t open the microphone。errcode :' + (error.code || error.name)
                  );
                  resp.msg = 'can not open resource';
                  break;
              }
      ...
      exchangeCamera: function (did, callback, onerror) {
        ...
        if (mediaConfig.voipCallData.connected && mediaConfig.voipCallData.localMediaStream) {
          ...
          var AudioTrack = null;
          if (
            commonConfig.peerConnection.getSenders &&
            (s = commonConfig.peerConnection.getSenders()) &&
            s.length > 0
          ) {
            ...
            mediaConfig.voipCallData.localMediaStream.removeTrack &&
            commonConfig.peerConnection.getSenders
          ) {
                  ...
                  Webrtc.createOffer(
                    function (e) {
                      ...
                      if (commonConfig.peerConnection.iceGatheringState === 'complete') {
                        ...
                        var msg = 'calling';
                        if (
                          mediaConfig.sendVoipData.callId ===
                          mediaConfig.voipCallData.callEventCallId
                        ) {
                          mediaMethods.buildCallEvent(mediaConfig.sendVoipData, callback, onerror);
                        } else {
                          state = 4;
      ...
      VideoRecord: function (local, isVideo) {
        ...
        var _stream =
          local === 'local' ? commonConfig.localMediaStream : commonConfig.remoteMediaStream;
      ...
      reqMeetingMemberVideo: function (data, callback, onError) {
        ...
        if (!data.memberId || !data.ssrc || !data.videoSource) {
          var resp = {};
          resp.code = commonConfig.errCode._WSG_REQVIDEO_MISS_PARAM;
          resp.msg = 'no need param, please check param';
          commonMethods.logger.info(
            'reqMeetingMemberVideo nessencery param. memberid:' +
              data.memberId +
              ',reqssrc:' +
              data.ssrc +
              ',videosource:' +
    ...
    this.__proto__ = {
      ...
      webrtcRealease: function (memberid) {
        if (commonConfig.peerConnectionMap[memberid]) {
          if (
            commonConfig.peerConnectionMap[memberid].signalingState !== 'closed' &&
      ...
      buildPeerConnection: function (callback, onError) {
        ...
        function addStream(event) {
          self.processAVStream(
            commonConfig.voipOtherView,
            event.streams ? event.streams[0] : event.stream
          );
        }
      ...
      getMediaRecorder: function () {
        var recorder =
          window.MediaRecorder || window.webkitMediaRecorder || window.moztMediaRecorder;
        if (recorder) {
          return recorder;
        } else {
          return false;
