(function (e) {
  ...
  YTX.prototype = {
    ...
    EV_sendTextMsg: function (content, callback) {
      ...
      let msgId = RL_YTX.sendChatroomMsg(
        obj,
        function (obj) {
          console.log(obj);
    ...
    EV_onMsgReceiveListener: function (obj) {
      ...
      console.log(obj);
      let roles = {
        1: '<i class="c_red">主播</i>',
        2: '<i class="c_green">管理员</i>',
        3: '<i class="c_normal">路人甲</i>',
      };
      if (obj.msgType === 1) {
        IM.addMsgToChatWindow(
          2,
          obj.senderNickName ? obj.senderNickName : obj.msgSender,
          obj.msgContent,
          obj.msgSender
        );
      }
    ...
    addMsgToChatWindow: function (level, name, content, userid) {
      if (level === 0) {
        ...
        var html =
          `<li data-userid="` +
          userid +
          `"><i class="icon icon_level level_` +
          level +
          `"></i><span data-sendername="` +
          name +
          `">` +
          name +
          `</span>: <b>` +
    ...
    addToOnlineList: function (arr) {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        html +=
          ...
          arr[i].type +
          '"  title="' +
          (!!arr[i].infoExt ? Base64.decode(arr[i].infoExt) : null) +
          '" data-entertime="' +
          arr[i].enterTime +
          '"><i class="icon icon_level level_1"></i><span  data-sendername="' +
          (arr[i].nickName ? arr[i].nickName : arr[i].userId) +
          '">[' +
          roles[arr[i].type] +
          '] <b>' +
          (arr[i].nickName ? arr[i].nickName : arr[i].userId) +
          '</b> <i>' +
          (arr[i].infoExt ? Base64.decode(arr[i].infoExt) : '') +
          '</i></span></li>';
      }
    ...
    setSilence: function (memberid, callback) {
      //禁言
    ...
    defriendMember: function (memberId, callback) {
      //拉黑
    ...
    forbidChatRoomMember: function (memberId, time, callback) {
      //禁言
    ...
    setAdministrator: function (memberId, callback) {
      ...
      RL_YTX.updateChatRoomMemberRole(
    ...
    modifyLiveMemberInfo: function (type, content, callback) {
      //修改个人信息
      ...
      RL_YTX.updateMyChatroomMemberInfo(
