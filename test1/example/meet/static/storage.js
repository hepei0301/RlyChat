  ...
  var prefix = 'p__hack_',
    ...
    encode = function (key) {
      ...
      return key
        ? Base64
          ? Base64.encode(key)
          : encodeURIComponent(prefix + key).replace(/%/g, spfix)
        : '';
    },
    decode = function (key) {
      key && (key = typeof key === 'string' ? key : JSON.stringify(key));
      return key
        ? Base64
          ? Base64.decode(key)
          : decodeURIComponent(key.replace(reg2, '%')).replace(reg1, '')
        : '';
    };
  ...
  (function () {
    ...
    LS = {
      ...
      getItem: function (key) {
        ...
        var value =
          (attrs.getNamedItem(encode(key)) || { nodeValue: null }).nodeValue ||
          root.getAttribute(encode(key));
