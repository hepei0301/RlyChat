(function (e) {
  ...
  var YTX_CONFIG = {
    ...
    _onResponse: function (obj) {
      ...
      if (!request) {
        log(
          ...
          'receive a unrequest response, clientNo:' + payload['head']['reqID']
        );
        return;
      }
        ...
        success: function (e) {
          if (!e) {
            YTX_CONFIG._log(
              ...
              'connect rest error, please check server ip and port' + e
            );
            var resp = {};
    ...
    _initScoket: function (callback, onError, reset) {
      ...
      if (!YTX_CONFIG._isConnect) {
        if (!YTX_CONFIG._isConnecting) {
          ...
          else serverip = 'ws://' + Base64.decode(YTX_CONFIG._server_ip[0]);
          ...
          YTX_CONFIG._socket.onopen = function (event) {
            ...
            if (!YTX_CONFIG._isReconnect)
              ...
              var joinRoomBuilder = new window.RL_WBSS_YTX.JoinRoomBuilder(
                YTX_CONFIG._roomId,
                YTX_CONFIG._roomPass,
                YTX_CONFIG._roleId
              );
          ...
          YTX_CONFIG._socket.onclose = function (event) {
            ...
            if (YTX_CONFIG._loginStatus == 2) {
              ...
              YTX_CONFIG._intervalId = window.setInterval(
                YTX_CONFIG._heartBeat,
                YTX_CONFIG._heartBeatInterval._RECONNECT * 1000
              );
    ...
    _heartBeatCallBackErr: function (times) {
      ...
      if (!YTX_CONFIG._isConnect || YTX_CONFIG._loginStatus == 1 || times > 3) {
        ...
        if (!YTX_CONFIG._failIntervalId) {
          YTX_CONFIG._failIntervalId = window.setInterval(
            YTX_CONFIG._heartBeat,
            YTX_CONFIG._failHeartBeatInterval * 1000
          );
        }
        if (!!YTX_CONFIG._intervalId) {
    ...
    _unInitSocket: function () {
      ...
      if (!!YTX_CONFIG._intervalId) window.clearInterval(YTX_CONFIG._intervalId);
    ...
    _protobuf: {
      ...
      _buildHeartBeat: function () {
        var id = setTimeout(
          YTX_CONFIG._heartBeatCallBackErr(++YTX_CONFIG._heartBeatErrNum),
          YTX_CONFIG._heartBeatTimeOut * 1000
        );
        return '{"hb":' + id + '}';
      },
      //绘制数据
      _buildDrawData: function (node) {
        ...
        YTX_CONFIG._sendMsg(
          '{"type":' + YTX_CONFIG._prototype.WbssDrawReq + ',"payload":' + sendJsonStr + '}'
        );
      },
      ...
      _buildCreateRoom: function (createRoomBuilder, callback, onError) {
        ...
        if (YTX_CONFIG._canvasAppManager) YTX_CONFIG._canvasAppManager.registerMouseEvent();
      ...
      _buildRemoveDoc: function (removeDocBuilder, callback, onError) {
        ...
        sendJsonStr = JSON.stringify(sendJsonStr);
        return (
          '{"type":' + YTX_CONFIG._prototype.RemoveShareDocReq + ',"payload":' + sendJsonStr + '}'
        );
      },
      //建立下一个白板
      _buildGotoPage: function (gotoPageBuilder, callback, onError) {
        ...
        YTX_CONFIG.updateRDP(
          gotoPageBuilder.getRoomId(),
          gotoPageBuilder.getDocId(),
          parseInt(gotoPageBuilder.getPageId())
        );
      ...
      _buildDeleteElement: function (drawId, callback, onError) {
        ...
        sendJsonStr = JSON.stringify(sendJsonStr);
        return (
          '{"type":' + YTX_CONFIG._prototype.WbssDeleteDrawReq + ',"payload":' + sendJsonStr + '}'
        );
      },
      ...
      _buildScale: function (scaleBuilder, callback, onError) {
        ...
        if (type == 0) {
          // 步长缩放
          log(
            YTX_CONFIG._logLev._DEBUG,
            'originalInfo={' + originalWidth + ',' + originalHeight + '}'
          );
        ...
        if (newViewWidth / pi.viewHeight < 0.1) {
          YTX_CONFIG._throwError(
            YTX_CONFIG._errcode._OUT_OF_RANGE_ERROR,
            'now is lease or max, out of range',
            onError
          );
          return;
        }
    ...
    _parseCreateRoomResp: function (obj, request) {
      ...
      if (respCode == 0) {
        ...
        YTX_CONFIG._intervalId = window.setInterval(
          YTX_CONFIG._heartBeat,
          YTX_CONFIG._heartBeatInterval._TEST_NETWORK * 1000
        );
        if (!!YTX_CONFIG._failIntervalId) {
    ...
    _parseGotoPageResp: function (obj, request) {
      ...
      if (!!request.callback) request.callback(resp);
    },
    ...
    _canvasApp: function () {
      ...
      this.appendElementNode = function (pi, node, isLocalHandle, isSelfData) {
        ...
        if (drawId in pi.elementNodeMap) {
          ...
          if (piNode._type == RL_WBSS_YTX._shapeType.FREELINE) {
            if (isLocalHandle || !isSelfData) piNode.appendCoordinates(node._coordinates);
      ...
      this.appendElementNode2 = function (pi, node, isLocalHandle, isSelfData) {
        ...
        if (drawId in pi.elementNodeMap) {
          ...
          if (piNode._type == RL_WBSS_YTX._shapeType.FREELINE) {
            ...
            if (node._isFinal && !isLocalHandle) {
              // 服务器的最终划线更新到map
              piNode._isValid = false;
            } else {
              // 中间过程线直接修改顶点数据
              if (isSelfData) piNode._coordinates[1].x = node._coordinates[1].x;
              piNode._coordinates[1].y = node._coordinates[1].y;
              return;
            }
          }
      ...
      function generateElementNode(eventSatus) {
        ...
        // 激光笔up状态设置无效
        if (
          YTX_CONFIG._drawContext.shapeType == RL_WBSS_YTX._shapeType.LASERPEN &&
          eventSatus == MOUSE_EVENT_STATUS.EVENT_UP
            ? true
            : false
        ) {
      ...
      this.generateTextNode = function (pos, text) {
        ...
        var strDrawId = pi.generateDrawId(
          YTX_CONFIG._userId,
          YTX_CONFIG._drawContext.curRoom,
          YTX_CONFIG._drawContext.curDoc
        );
          ...
          case RL_WBSS_YTX._shapeType.FREELINE:
            {
              ...
              for (var i = 1; i < element._coordinates.length; i++) {
                if (element._coordinates[i].x < leftX) leftX = element._coordinates[i].x;
                if (element._coordinates[i].x > rightX) rightX = element._coordinates[i].x;
                if (element._coordinates[i].y > bottomY) bottomY = element._coordinates[i].y;
                if (element._coordinates[i].y < topY) topY = element._coordinates[i].y;
              }
              pos1.x = leftX;
              pos1.y = topY;
              pos2.x = rightX;
      ...
      function handleIsSelectOnPoint(pos) {
        ...
        for (var i = pi.elementNodeList.length - 1; i >= 0; i--) {
          ...
          if (!element || !element._isValid || element._isUndo) continue;
      ...
      function doMouseDown(event) {
        ...
        if (YTX_CONFIG._drawContext.drawStatus == YTX_CONFIG._drawStatus.DRAW_STATUS_DELETE) {
          ...
          log(YTX_CONFIG._logLev._DEBUG, 'drawId = ' + drawId);
          if (drawId != '') {
            var sendStr = YTX_CONFIG._protobuf._buildDeleteElement(
              drawId,
              function () {},
              function () {}
            );
            if (!!sendStr) YTX_CONFIG._sendMsg(sendStr);
          }
          return;
        }
      ...
      function doMouseMove(event) {
        if (mouseEventStatus != MOUSE_EVENT_STATUS.EVENT_NONE) {
          ...
          if (nowTime - lastTime > 50) {
            ...
            // 元素选中移动模式
            if (
              YTX_CONFIG._drawContext.drawStatus == YTX_CONFIG._drawStatus.DRAW_STATUS_ELEMENT_MOVE
            ) {
              return;
            }
      ...
      function screenToWorldCoors(pi, coors) {
        ...
        if (YTX_CONFIG._drawContext.curDoc >= 0) {
          for (var i = 0; i < coors.length; i++) {
            coors[i].x =
              Math.round(1000 * ((2.0 * (coors[i].x - pi.fromX)) / pi.viewWidth - 1.0)) / 1000; //.toFixed(3);
            coors[i].y =
              Math.round(1000 * (1.0 - (2.0 * (coors[i].y - pi.fromY)) / pi.viewHeight)) / 1000; //.toFixed(3);
          }
        }
      }
      ...
      function clearCanvas() {
        ...
        context.clearRect(
          0,
          0,
          YTX_CONFIG._drawContext.canvasLastWidth,
          YTX_CONFIG._drawContext.canvasLastHeight
        );
      ...
      this.drawScreen = function () {
        ...
        if (YTX_CONFIG._drawContext.curDoc > 0) {
          ...
          if (!pi.image) {
            ...
            // return;
            var key =
              YTX_CONFIG.getCurRoomId().toString() +
              '-' +
              YTX_CONFIG.getCurDocId().toString() +
              '_' +
              YTX_CONFIG.getCurPageId().toString() +
              '.png';
            YTX_CONFIG.getdata(
              YTX_CONFIG._dbInfo.dbStoreName,
              key,
              function (data) {
                log(
                  YTX_CONFIG._logLev._DEBUG,
                  'draw(' + YTX_CONFIG._drawCnt + ') ' + 'getdata ' + key + ' success'
                );
                var blob = data['imagedata'];
                blobToImage(blob, function (img) {
                  pi.image = img;
                  YTX_CONFIG.setDocViewPosition(
                    YTX_CONFIG.getCurRoomId(),
                    YTX_CONFIG.getCurDocId(),
                    img.width,
                    img.height
                  );
        ...
        for (var i = 0; i < pi.elementNodeList.length; i++) {
          ...
          context.lineWidth = getZoomWidth(
            element._lineWidth,
            element._drawAreaWidth,
            pi.viewWidth
          );
            ...
            case RL_WBSS_YTX._shapeType.CIRCLE:
              {
                context.beginPath();
                context.arc(
                  (coors[1].x + coors[0].x) / 2,
                  (coors[1].y + coors[0].y) / 2,
                  Math.sqrt(
                    Math.pow(coors[1].x - coors[0].x, 2) + Math.pow(coors[1].y - coors[0].y, 2)
                  ) / 2,
                  0,
                  2 * Math.PI
                );
            ...
            case RL_WBSS_YTX._shapeType.FONT:
              {
                ...
                context.scale(
                  pi.viewWidth / element._drawAreaWidth,
                  pi.viewWidth / element._drawAreaWidth
                );
    ...
    _rePaint: function () {
      if (YTX_CONFIG._canvasAppManager) YTX_CONFIG._canvasAppManager.drawScreen();
    },
    ...
    syncDrawData: function (roomId, docId, curDocTimestamp) {
      ...
      if (curDocTimestamp > localDocTimestamp) {
        ...
        sendJsonStr = JSON.stringify(sendJsonStr);
        var sendStr =
          '{"type":' + YTX_CONFIG._prototype.SyncRoomDataReq + ',"payload":' + sendJsonStr + '}';
    ...
    setElementNodeDrawUndo: function () {
      ...
      for (var ri = pi.elementNodeList.length - 1; ri >= 0; ri--) {
        ...
        if (!element || element._isValid == false || element._isUndo) continue;
    ...
    asyncDownfile: function (roomId, docId, pageId, reqId) {
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
          log(
            YTX_CONFIG._logLev._DEBUG,
            'Image(' + roomId + ',' + docId + ',' + pageId + ') download success'
          );
    ...
    checkIfReGetViewInfo: function (roomId, docId) {
      ...
      if (
        newWidth != YTX_CONFIG._drawContext.canvasLastWidth ||
    ...
    isSamePage: function (roomId, docId, pageId) {
      return (
        YTX_CONFIG.getCurRoomId() == roomId &&
        YTX_CONFIG.getCurDocId() == docId &&
        YTX_CONFIG.getCurPageId() == pageId
      );
    },
    ...
    DocInfo: function (docId, docName, pageSize, docUrl, owner) {
      ...
      this.setDocViewPosition = function (imageWidth, imageHeight) {
        if (!this.isDocViewInit) {
          ...
          log(
            YTX_CONFIG._logLev._DEBUG,
            'DocInitViewPostion(' + imageWidth + ',' + imageHeight + ')'
          );
          var fromX, fromY, viewWidth, viewHeight;
          ...
          log(
            YTX_CONFIG._logLev._DEBUG,
            'viewInfo(' + fromX + ',' + fromY + ',' + viewWidth + ',' + viewHeight + ')'
          );
  ...
  window.RL_WBSS_YTX = window.RL_WBSS_YTX || {
    ...
    uploadDoc: function (fileInfo, callback, onError, process) {
      ...
      reader.onload = function loaded(evt) {
        ...
        xhr.setRequestHeader(
          'Content-Disposition',
          'attachment;filename=' + encodeURI(fileInfo.name)
        );
        xhr.setRequestHeader('Ytx-Params', Base64.encode(JSON.stringify(params)));
    ...
    drawUndo: function (type, callback, onError) {
      //撤销，应该在这判断是否是当前用户，未添加
