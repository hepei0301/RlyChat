...
*/
  ...
  List.prototype = {
    ...
    tryChangeActive: function tryChangeActive(e) {
      ...
      var $elem = this.$elem;
      if (
        editor.cmd.queryCommandState('insertUnOrderedList') ||
        editor.cmd.queryCommandState('insertOrderedList')
      ) {
        this._active = true;
        $elem.addClass('w-e-active');
      } else {
...
*/
  ...
  function Emoticon(editor) {
    this.editor = editor;
    let title =
      window.CurrentUser && window.CurrentUser.systemSettings
        ? window.CurrentUser.systemSettings.Language == 'en'
          ? 'emoji'
          : '表情'
        : '表情';
    this.$elem = $(
      '<div class="w-e-menu" title="' +
        title +
  ...
  Emoticon.prototype = {
    ...
    onClick: function onClick() {
      this.editor.customConfig.emojiClick && this.editor.customConfig.emojiClick();
      this._createPanel();
    },
...
*/
  ...
  function Image(editor) {
    ...
    this.$elem = $(
      '<div class="w-e-menu" id="' + imgMenuId + '"><i class="w-e-icon-image"></i></div>'
    );
...
*/
  ...
  function getPasteHtml(e, filterStyle, ignoreImg) {
    ...
    console.log('isBase64', isBase64(pasteText));
    pasteText = pasteText
      .replace(/</gm, '&lt;')
      .replace(/>/gm, '&gt;')
      .replace(/"/gm, '&quot;')
    ...
    if (!pasteHtml && pasteText) {
      // if(isBase64(pasteText)){
      //     pasteHtml = '<p><img src=' + pasteText + ' style="max-width:100%;"/></p>';
      // }else{
      // }
      pasteText = pasteText.length <= 1024 ? pasteText : pasteText.substr(0, 1024);
      var reg = /<(?:(?:\/?[A-Za-z]\w*\b(?:[=\s](['"]?)[\s\S]*?\1)*)|(?:!--[\s\S]*?--))\/?>/g; //验证规则
      if (pasteText.match(reg)) {
  ...
  Text.prototype = {
    ...
    _enterKeyHandle: function _enterKeyHandle() {
      ...
      $textElem.on('keyup', function (e) {
        if (e.keyCode !== 13) {
          // 不是回车键
          editor.customConfig.normaldown && editor.customConfig.normaldown(e);
          return;
        }
        // 将回车之后生成的非 <p> 的顶级标签，改为 <p>
        pHandle(e);
      ...
      $textElem.on('keydown', function (e) {
        ...
        editor.customConfig.keydown && editor.customConfig.keydown(e);
        // console.log(editor)
        // <pre><code></code></pre> 回车时 特殊处理
        // codeHandle(e);
      });
    ...
    _pasteHandle: function _pasteHandle() {
      ...
      $textElem.on('paste', function (e) {
        ...
        if (!pasteHtml || !!hasFile(e)) {
          // 没有内容或者粘贴板中有文件，可继续执行下面的图片粘贴
          resetTime();
          return;
        }
        try {
          ...
          if (isBase64(pasteText)) {
            editor.cmd.do('insertHTML', '<img src="' + pasteText + '" style="max-width:100%;"/>');
          } else {
            let flag = true;
            pasteHtml = editor._transformEmoJiCharacter(pasteHtml); //粘贴表情
            var reg = /<(?:(?:\/?[A-Za-z]\w*\b(?:[=\s](['"]?)[\s\S]*?\1)*)|(?:!--[\s\S]*?--))\/?>/g; //验证规则
            if (pasteHtml.match(reg)) flag = false;
            editor.cmd.do('insertHTML', pasteHtml, flag);
          }
        } catch (ex) {
...
*/
  ...
  Command.prototype = {
    ...
    // 执行命令
    do: function _do(name, value, bool) {
      var editor = this.editor;
      ...
      if (this[_name]) {
        // 有自定义事件
        this[_name](value, bool);
      } else {
        // 默认 command
        this._execCommand(name, value, bool);
      }
    ...
    // 自定义 insertHTML 事件
    _insertHTML: function _insertHTML(html, bool) {
      var editor = this.editor;
      ...
      if (this.queryCommandSupported('insertHTML')) {
        // W3C
        if (!!bool) {
          this._execCommand('insertText', html);
        } else {
    ...
    uploadImg: function uploadImg(files) {
      ...
      if (uploadImgServer && typeof uploadImgServer === 'string') {
        ...
        if (hooks.before && typeof hooks.before === 'function') {
          ...
          if (
            beforeResult &&
            (typeof beforeResult === 'undefined' ? 'undefined' : _typeof(beforeResult)) === 'object'
          ) {
...
*/
  ...
  Editor.prototype = {
    ...
    // 表情转换
    _transformEmoJiCharacter: function (emoJiStr) {
      for (let i in emoji.show_dataAuto) {
        let item = emoji.show_dataAuto[i];
        let imgStr =
          '<img class="emoji" src="./static/img-apple-64/' +
          item.unicode +
          '.png" height="20" width="20" style="vertical-align: middle;" align="absmiddle">';
        let reg = new RegExp('\\' + item.icon, 'g');
