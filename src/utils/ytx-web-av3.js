...
//=======================================Meeting==============================//
  ...
  YTX_Meeting.handleRequest = function(){
    ...
    switch(request.messageId){
      ...
      case YTX_MessageId.StartScreenShare:
        ...
        YTX_Meeting.startScreenShare(request.LayoutIdx,request.onSuccess, request.onFail);
        break;
      case YTX_MessageId.CancelScreenShare:
        console.log(YTX_Meeting.getTimeStamp(), "[Meeting] Handle Cancel ScreenShare");
        YTX_Meeting.cancelScreenShare('50');
        break;
    }
  };
  ...
  YTX_Meeting.unSubscribeMediaFail = function(request, err){
    console.log(YTX_Meeting.getTimeStamp(), "[Meeting] unSubscribe Media Failed, userName:", request.userName, ", userId:", request.userId,
  ...
  YTX_Meeting.startScreenShareSuccess = function(){
    ...
    sessionStorage.setItem('isSelfSharing',1)
    YTX_Meeting.connState = CState.Connected;
  ...
  YTX_Meeting.cancelScreenShareFail = function(err){
    console.log(YTX_Meeting.getTimeStamp(), "[Meeting] Cancel ScreenShare Failed, errName:", err.name, ", errMessage:", err.message);
    YTX_Meeting.handleRequest();
  };

  YTX_Meeting.cancelScreenShareSuccess = function(){
    console.log(YTX_Meeting.getTimeStamp(), "[Meeting] Cancel ScreenShare Success");
    sessionStorage.setItem('isSelfSharing',0)
    YTX_Meeting.handleRequest();
  };
/////////////////////////////////////////////////////////////////////////////////////////////
  ...
  YTX_Meeting.processCallMessage = function(msgLite){
    ...
    // console.log(msgLite,'msgLitemsgLitemsgLite')
  ...
  YTX_Meeting.makeCall = function(parameter, onSuccess, onFail){
    ...
    if(YTX_Meeting.mediaType == MType.Audio){
      ...
      .then(function(sdp){
        ...
        console.log(YTX_Meeting.getTimeStamp(), "[Meeting] MakeCall, sendInvite for audio.");
      })
      .catch(function(err){
        YTX_Meeting.makeCallFail(err);
      })
      console.log(YTX_Meeting.getTimeStamp(), "[Meeting] MakeCall2, sendInvite for audio.");
     
    }else{
      YTX_Webrtc.getLocalAudioStream()
      .then(function(stream){
        // console.log(YTX_Meeting.getTimeStamp(), "[Meeting] MakeCall00, sendInvite for audio.");
        YTX_Meeting.startLocalMedia(stream, MType.Audio);
        YTX_Webrtc.addLocalAudioStream(stream);
        return YTX_Webrtc.getLocalVideoStream()
      })
      .then(function(stream){
        // console.log(YTX_Meeting.getTimeStamp(), "[Meeting] MakeCall11, sendInvite for audio.");
        YTX_Meeting.startLocalMedia(stream, MType.Video);
        YTX_Webrtc.addLocalVideoStream(stream);
        return YTX_Webrtc.createLocalOffer()
      })
      .then(function(sdp){
        // console.log(YTX_Meeting.getTimeStamp(), "[Meeting] MakeCall33, sendInvite for video.");
        YTX_Meeting.sendInvite(sdp);
      })
      .catch(function(err){
        YTX_Meeting.makeCallFail(err);
      })
      // console.log(YTX_Meeting.getTimeStamp(), "[Meeting] MakeCall22, sendInvite for audio.");
    }
  };
//////////////////////////////////////
...
/////////////////////////////screen share////////////////////////////
  ...
  YTX_Meeting.startScreenShare = function(LayoutIdx,onSuccess, onFail){
    console.log(LayoutIdx,'LayoutIdxLayoutIdx')
    ...
    .then(function(sdp){
      YTX_Meeting.sendPublishMediaRequest(LayoutIdx,sdp);
    })
    .catch(function(err){
  ...
  YTX_Meeting.sendPublishMediaRequest = function(LayoutIdx,sdp){
    console.log(LayoutIdx,'LayoutIdxLayoutIdx')
    let customData = {};
    let clientNo = YTX_Meeting.clientNo++;
    ...
    customData.LayoutIdx = LayoutIdx;
    customData.selfssrc = YTX_Meeting.selfSsrc;
...
//////////////////////////////////////
  YTX_Meeting.cancelScreenShare = function(LayoutIdx){
    let customData = {};
    let clientNo = YTX_Meeting.clientNo++;
...
//==================================webrtc=====================================//
  ...
  YTX_Webrtc.videoWidth = 1280;
  YTX_Webrtc.videoHeight = 720;
  ...
  YTX_Webrtc.changeVideoInput = function(deviceId, onSuccess, onFail){
    let constraints = {
      ...
      video: {
        deviceId: deviceId,
        width: { exact: YTX_Webrtc.videoWidth },
        height: { exact: YTX_Webrtc.videoHeight },
        frameRate: { max: YTX_Webrtc.videoFrameRate }
      }
    }
    navigator.mediaDevices.getUserMedia(constraints)
    .then(function(stream) {
      console.log(stream,'streamsuccess')
  ...
  YTX_Webrtc.changeVideoInputByStream = function(stream,tark){
          window.stream = stream
          console.log(stream.getVideoTracks()[0],'streamstreamstreamstreamstream')
          YTX_Webrtc._videoRtpSender.track.stop();
          YTX_Webrtc._videoRtpSender.replaceTrack(stream.getVideoTracks()[0]);
          console.log(YTX_Meeting.getTimeStamp(), "[Meeting] changeWhileBoardVideoInput success");
          return stream
  };
  ...
  YTX_Webrtc.getLocalAudioStream = function(){
    ...
    return new Promise(function(resolve, reject){
      console.log(YTX_Meeting.getTimeStamp(), "[Meeting] Get Local AudioStream begin...");
  ...
  YTX_Webrtc.getLocalVideoStream = function(){
    console.log(YTX_Meeting.getTimeStamp(), "[Meeting] Get Local VideoStream begin...");
  ...
  YTX_Webrtc.createScreenOffer = function(){
    return new Promise(function(resolve, reject){
      ...
      // console.log(YTX_Webrtc._peerConnection,'_peerConnection')
      // console.log(JSON.stringify(YTX_Webrtc._peerConnection.remoteDescription)  ,'_peerConnection')
      YTX_Webrtc._peerConnection.createOffer()
      .then(function(offer){
...
/*****************************************接入新版本sdk*************************************************/
  ...
  function RL_Media() {
      ...
            YTX_Meeting.setOnUnSubscribeMediaSuccess(callback);
          ...
          startScreenShare: function (LayoutIdx,onSuccess, onFail) {
            let request = {};
            request.onFail = onFail;
            request.onSuccess = onSuccess;
            request.messageId = YTX_MessageId.StartScreenShare;
            request.LayoutIdx = LayoutIdx.LayoutIdx
            console.log(YTX_Meeting.getTimeStamp(), "[Meeting] Add ScreenShareRequest");
            YTX_Meeting.addRequest(request)
          },
          cancelScreenShare:function(LayoutIdx){
            YTX_Meeting.cancelScreenShare(LayoutIdx);
          },
          ////////////////////////////////////////////////////
          stopSreenShare:function(){
            YTX_Webrtc.stopSreenShare()
          },
          ...
          changeVideoInputByStream: function(stream,tark){
            YTX_Webrtc.changeVideoInputByStream(stream,tark);
          },
